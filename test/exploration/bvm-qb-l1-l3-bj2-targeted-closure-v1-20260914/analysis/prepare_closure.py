#!/usr/bin/env python3
"""Register the targeted three-parameter BVM array closure experiment."""
...
PREVIOUS = REPO / "test/exploration/bvm-qb-l3-bj2-targeted-combination-v1-20260911"
PREVIOUS_L3 = REPO / "test/exploration/bvm-qb-l3-highside-selective-window-v1-20260911"
...
POINTS = (("TARGET_L1_1p2_L3_2_BJ2_2p2", OrderedDict((("L1_pH", 1.2), ("L3_pH", 2.0), ("BJ2_area", 2.2)))),)
MASKS = ("0011", "0111")
...
def make_variant(changes: OrderedDict[str, float]) -> str:
    ...
    replacements = {
        "L1_pH": ("L1 2 3 {L1_VALUE}", "L1 2 3 {value:g}p"),
    ...
    for parameter, (old, new) in replacements.items():
        if parameter in changes:
            ...
            text = text.replace(old, new.format(value=changes[parameter]), 1)
    return text
...
def build_deck(point_id: str, mask: str, variant: str) -> str:
    ...
    if base.count(".param L1_VALUE=1.4p") != 1:
        raise RuntimeError("baseline L1 parameter is not unique")
    base = base.replace(".param L1_VALUE=1.4p", ".param L1_VALUE=1.2p", 1)
    ...
    return base.replace(base.splitlines()[0], f"* GENERATED FULL CLOSED-LOOP L1/L3/BJ2 CLOSURE DECK: {point_id}; mask={mask}", 1)
...
def deck_check(path: Path, point_id: str, mask: str, variant: str) -> dict[str, Any]:
    ...
    if ".param L1_VALUE=1.2p" not in text:
        failures.append("target L1 parameter not bound")
    variant_text = (EXP / "inputs/bq_variants" / variant).read_text(encoding="utf-8")
    if "L1 2 3 1.2p" not in variant_text or "BJ2 4 0 jjmit area=2.2" not in variant_text or "L3 4 OUT 2p" not in variant_text:
        failures.append("target variant does not bind L1=1.2, BJ2=2.2 and L3=2.0")
    ...
    variant_text = (EXP / "inputs/bq_variants" / variant).read_text(encoding="utf-8")
    if "L1 2 3 1.2p" not in variant_text or "BJ2 4 0 jjmit area=2.2" not in variant_text or "L3 4 OUT 2p" not in variant_text:
        failures.append("candidate variant does not bind L1=1.2, BJ2=2.2 and L3=2.0")
...
def main() -> int:
    ...
    reference_origins = {
        "canonical_l3p13/0011": PREVIOUS_L3 / "references/canonical_l3p13/0011",
        "canonical_l3p13/0111": PREVIOUS_L3 / "references/canonical_l3p13/0111",
        "l1p12_l3p13_bj2p20/0011": PREVIOUS_OFAT / "runs/SCREEN_L1_pH_1p2_0011",
        "l1p12_l3p13_bj2p20/0111": PREVIOUS_OFAT / "runs/SCREEN_L1_pH_1p2_0111",
        "l3p20_bj2p22/0011": PREVIOUS / "runs/TARGET_L3_2_BJ2_2p2_0011",
        "l3p20_bj2p22/0111": PREVIOUS / "runs/TARGET_L3_2_BJ2_2p2_0111",
    }
    references = {}
    ...
    previous_package = PREVIOUS / "handoff/bvm-qb-l3-bj2-targeted-combination-v1-20260911_raw_handoff.zip"
    l3_package = PREVIOUS_L3 / "handoff/bvm-qb-l3-highside-selective-window-v1-20260911_raw_handoff.zip"
    ofat_package = PREVIOUS_OFAT / "handoff/bvm-full-closed-loop-qb-parameter-screening-v1-20260911_raw_handoff_v2.zip"
    source_manifest = {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-source-manifest-v1", "experiment_id": EXP.name, "created_at_local": now(), "repository_head_at_registration": head(), "remote_master_at_registration": remote_head(), "inputs": inputs, "references": references, "variants": [artifact(REPO / point["variant_path"], "targeted L1/L3/BJ2 BQ variant") for point in points], "previous_combination_package": artifact(previous_package, "previous L3/BJ2 combination evidence package"), "previous_l3_package": artifact(l3_package, "previous L3 high-side evidence package"), "previous_ofat_package": artifact(ofat_package, "previous OFAT evidence package"), "baseline": BASELINE, "target_changes": {"L1_pH": 1.2, "L3_pH": 2.0, "BJ2_area": 2.2}, "solver": {"path": SOLVER.relative_to(REPO).as_posix(), "sha256": sha256(SOLVER), "version": subprocess.check_output([str(SOLVER), "--version"], cwd=REPO, text=True)}, "logical_new_point_count": 1, "logical_new_case_count": 2, "maximum_new_screening_solve_count": 2, "maximum_total_new_physical_solve_count_if_later_scientific_authorization": 4, "conditional_validation_masks": list(VALIDATION_MASKS), "conditional_validation_status": "RESERVED_REQUIRES_SCIENTIFIC_REVIEW", "no_replay": True, "no_passive_capture": True, "no_source_scaling": True, "no_jtl_change": True, "no_timestep_change": True}
    source_path = EXP / "SOURCE_MANIFEST.json"
    write_once(source_path, json.dumps(source_manifest, ensure_ascii=False, indent=2) + "\n", args.refresh_head)
    matrix = {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-matrix-v1", "experiment_id": EXP.name, "created_at_local": now(), "baseline": BASELINE, "target_changes": {"L1_pH": 1.2, "L3_pH": 2.0, "BJ2_area": 2.2}, "points": points, "masks": list(MASKS), "logical_new_point_count": 1, "logical_new_case_count": 2, "maximum_new_screening_solve_count": 2, "conditional_validation_masks": list(VALIDATION_MASKS), "validation_condition": "only after explicit scientific review authorization", "failures": failures}
    matrix_path = EXP / "screening/CLOSURE_MATRIX.json"
    write_once(matrix_path, json.dumps(matrix, ensure_ascii=False, indent=2) + "\n", args.refresh_head)
    table = "\n".join(["# L1/L3/BJ2 targeted full closed-loop closure matrix", "", "| point | L1 | L3 | BJ2 area | masks | use |", "|---|---:|---:|---:|---|---|", "| TARGET_L1_1p2_L3_2_BJ2_2p2 | 1.2 pH | 2.0 pH | 2.2 | 0011, 0111 | registered screening pair |", "", "References: canonical, L1-only rescue, and previous L3/BJ2 barrier point are immutable context only.", "", "Maximum new screening solves: 2. Masks 0001 and 1111 are reserved for a later scientifically authorized validation task and are not authorized here.", ""])
    write_once(EXP / "screening/CLOSURE_TABLE.md", table, args.refresh_head)
    previous = json.loads((EXP / "analysis/preflight.json").read_text(encoding="utf-8")) if (EXP / "analysis/preflight.json").is_file() else {}
    record = {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-preflight-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": "PASS" if not failures else "FAIL", "registration_head": previous.get("registration_head", head()), "head_at_preflight": head(), "remote_master_at_preflight": remote_head(), "logical_new_point_count": 1, "logical_new_case_count": 2, "maximum_new_screening_solve_count": 2, "authorized_points": [point_id_value for point_id_value, _changes in POINTS], "authorized_screening_masks": list(MASKS), "conditional_validation_masks": list(VALIDATION_MASKS), "validation_condition": "requires explicit scientific review authorization; not authorized in this experiment", "phase_baseline_window_ps": [101.0, 110.0], "phase_navigation_thresholds_turns": [0.5, 1.5, 2.5, 3.5, 4.5], "corrected_control_windows_ps": [[70.0, 81.0], [81.0, 90.0], [90.0, 101.0], [101.0, 110.0]], "rollback_excluded_from_control": True, "physical_solve_count_before_preflight": 0, "scientific_analysis_performed": False, "n1_n4_validation_authorized": "NOT_AUTHORIZED_WITHOUT_SCIENTIFIC_REVIEW", "matrix_sha256": sha256(matrix_path), "source_manifest_sha256": sha256(source_path), "failures": failures}
    preflight_path = EXP / "analysis/preflight.json"
    write_once(preflight_path, json.dumps(record, ensure_ascii=False, indent=2) + "\n", args.refresh_head)
    preflight_md = "\n".join([
        "# BVM QB L1/L3/BJ2 targeted closure - PREFLIGHT", "", CONTRACT_SENTENCE, "",
        ...
        "## Exact scope", "", "The only new screening point is L1=1.2 pH plus L3=2.0 pH plus BJ2 area=2.2, with masks 0011 and 0111. Frozen parameters are Lin=1.5 pH, L2=2.0 pH, RJ1=32 ohm, RJ2=12 ohm, IBias=260 uA and BJS/BJ1 areas 4.0/0.9. Three reference pairs are immutable context and are not rerun.", "",
        "## Source and topology", "", "The resolved closure is inputs/jjmit.cir + inputs/bvm_jm2_connected.cir + the target BQ variant + inputs/jtl2.cir; topology is BVM1..4 -> COMMON_SL -> eight-JJ JSL -> QB -> six-stage JTL -> R_TERM. The BQ variant changes only L1 1.4 -> 1.2 pH, BJ2 area 2.0 -> 2.2 and L3 1.3 pH -> 2.0 pH.", "",
        ...
        "## Conditional validation", "", "No third solve is authorized before scientific review. Masks 0001 and 1111 are reserved for a separate later validation task only after a clean ordered 2/3 result and explicit scientific-review authorization. This experiment maximum is two new physical solves.", "",
        ...
        "## Stop", "", "After the two screening solves, stop at EXPERIMENT_COMPLETE / AWAITING_SCIENTIFIC_REVIEW. A clean 2/3 result is a candidate record only; it does not authorize a third solve in this experiment.", "",
    ])
