...
- 任务合同、CLAUDE mailbox 消息和 receipt 只给出完成当前任务所需的上下文：目标、边界、验收、相关文件、命令/日志路径和失败摘录；不转发完整聊天记录、整个仓库或冗长原始日志。
- 测试通过时，记录命令、退出码、版本和证据路径即可；测试失败时先保留最小失败摘录、日志路径与复现命令，再请求工程审阅，而不是让多个代理重复通读日志。
