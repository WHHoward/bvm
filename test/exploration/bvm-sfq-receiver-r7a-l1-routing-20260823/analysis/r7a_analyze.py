...
RAW = RUN / "raw"
OUT = RUN / "analysis"
R6B = REPO / "test/exploration/bvm-sfq-receiver-r6b-native-qb-ratio-20260822"
...
def monotonic_segments(rows, phase_key, voltage_key, window):
    ...
    if len(active) < 3:
        return []
    # Match the accepted R6-B descriptive segment algorithm so the direct
    # L1 comparison does not change the segmentation rule.
    half = 5
...
def main():
    ...
    r6b_infos = {case: load_case(R6B, case, "run-01.csv") for case in cases}
    r6b_routing = {
        "read1": routing(r6b_infos["read1"], r6b_infos["logical1-read0-control"]),
        "read0": routing(r6b_infos["read0"], r6b_infos["logical0-read0-control"]),
    }
    ...
    output = {
        "analysis": {
            ...
            "R6B_reference_routing": r6b_routing,
            "raw_sha256": {
        ...
        "R6B_cases": {case: case_result(r6b_infos[case]) for case in cases},
    ...
    output_path = OUT / "r7a-summary.json"
