...
for mask in "${MASKS[@]}"; do
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    echo "JoSIM failed for $mask; immutable artifacts preserved" >&2
    exit "$exit_code"
  fi
  if grep -Eqi 'Missing model:|Using default model|error|fatal' "$log"; then
    echo "solver/model warning for $mask; artifacts preserved" >&2
    exit 6
  fi
...
echo "completed exactly ten common-SL -> 12-JSL -> QB -> JTL runs"
