#!/usr/bin/env python3
"""Execute exactly the six newly authorized L2 runs once."""
...
from prepare_experiment import FIXED, NEW_RUNS, REUSE_RUNS, RUN_TO_L2, RUN_TO_MASK  # noqa: E402
...
def make_record(run_id: str, head: str, command: list[str], started: str, finished: str, exit_code: int, deck: Path, raw: Path, log: Path) -> dict[str, Any]:
    return {
        "schema": "bjs400-l2-rearm-run-metadata-v1",
        "experiment_id": EXP.name,
        "run_id": run_id,
        "topology": "ARRAY",
        "working_point": {"L1_pH": FIXED["L1_pH"], "IBias_uA": FIXED["IBias_uA"], "RJ1_ohm": FIXED["RJ1_ohm"], "L2_pH": RUN_TO_L2[run_id]},
...
def main() -> int:
    ...
    summary = {
        "schema": "bjs400-l2-rearm-execution-v1",
        "experiment_id": EXP.name,
