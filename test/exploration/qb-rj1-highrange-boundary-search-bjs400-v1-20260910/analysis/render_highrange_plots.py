#!/usr/bin/env python3
"""Render compact whole-run plots for the high-range RJ1 evidence."""
...
RUN_PATHS.update({case: EXP / "references/historical" / case for case in HISTORICAL_CASES})
TREND_CASES = {
    "0001": ["RJ10_0001", "RJ12_0001", "RJ14_0001", "RJ16_0001"] + [f"ARRAY_L1P16_IB260_RJ{rj1}_0001" for rj1 in (20, 24, 32, 48)],
    "0011": ["RJ10_0011", "RJ12_0011", "RJ14_0011", "RJ16_0011"] + [f"ARRAY_L1P16_IB260_RJ{rj1}_0011" for rj1 in (20, 24, 32, 48)],
}
...
def render_comparison(entries: list[dict[str, object]], name: str, case_ids: list[str]) -> None:
    ...
    temp_path = Path(tempfile.mkstemp(prefix="rj1-highrange-", suffix=".csv", dir="/tmp")[1])
    try:
...
def write_run_summaries() -> list[Path]:
    ...
    for run_id in NEW_RUNS:
        ...
        lines = [
            f"# {run_id} — high-range RJ1 evidence",
            "",
...
def main() -> int:
    ...
    render_comparison(entries, "RJ1_FULL_TREND_0001", TREND_CASES["0001"])
    render_comparison(entries, "RJ1_FULL_TREND_0011", TREND_CASES["0011"])
    ...
    manifest = {
        "schema": "rj1-highrange-flat-visualization-manifest-v1",
        ...
        "historical_rj1_values_ohm": [10, 12, 14, 16],
        "new_rj1_values_ohm": [20, 24, 32, 48],
        "masks": ["0001", "0011"],
        "scientific_analysis_performed": False,
        "no_focused_window_plots": True,
        "no_extra_comparison_plots": True,
        "status": "PASS" if len(standalone) == 24 and len(comparisons) == 2 else "FAIL",
    }
