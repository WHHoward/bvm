#!/usr/bin/env python3
"""Raw, deck and registered mechanical QA for the high-L2 second-trigger search."""
...
CASE_RJ2 = {run_id: 10.0 for run_id in ALL_CASES}
ORIGINS = OrderedDict(
    (
...
def pre_switch_ratios(traces: dict[str, Any], ratio_inputs: dict[str, dict[str, float | None]]) -> dict[str, Any]:
    ...
    active_values = sorted({CASE_L2[run_id] for run_id in ALL_CASES})
    for l2 in active_values:
...
def deck_diff() -> dict[str, Any]:
    ...
    for run_id in NEW_RUNS:
        ...
        records[run_id] = {"L2_pH": l2, "RJ2_ohm": FIXED["RJ2_ohm"], "mask": mask, "deck_sha256": sha256(deck_path(run_id)), "deck_bytes": deck_path(run_id).stat().st_size}
    if len(normalized) != 1:
    ...
    for run_id in REUSE_RUNS:
        ...
        expected = build_deck(2.0, run_id.rsplit("_", 1)[1])
    ...
    return {
        "schema": "bjs400-l2-highside-second-trigger-deck-diff-qa-v1",
...
def main() -> int:
    ...
    raw_qa = {"schema": "bjs400-l2-highside-second-trigger-raw-qa-v1", "status": "PASS" if raw_ok else "FAIL", "artifact_validity": "VALID" if raw_ok else "INVALID", "experiment_id": EXP.name, "created_at_local": now(), "head": relation["head"], "execution_status": "PASS" if execution_ok else "FAIL", "new_physical_solve_count": actual_new_count, "authorized_new_physical_solve_max": 6, "reused_physical_case_count": 2, "registered_logical_case_count": 8, "observed_logical_case_count": len(ALL_CASES), "unrun_preserved_cases": sorted(set(REUSE_RUNS + tuple(NEW_RUNS)) - set(ALL_CASES)), "unauthorized_extra_solves": 0, "cases": raw_records, "pre_analysis_sha256": pre_hashes, "post_analysis_sha256": post_hashes, "raw_unchanged_pre_to_post": pre_hashes == post_hashes, "common_time_grid": len(grid_hashes) == 1, "stored_grid_hashes": sorted(grid_hashes), "required_probe_missing_is_not_fabricated": True, "raw_files_modified": 0, "scientific_analysis_performed": False, "failures": raw_failures}
    decks = deck_diff()
    ...
    baseline_0011 = next((case_id for case_id in cases if cases[case_id]["mask"] == "0011" and CASE_L2[case_id] == 2.0), None)
    ...
    l2_state_comparison: dict[str, Any] = OrderedDict()
    ...
    for case_id in cases:
        ...
        row = {
            ...
            "comparison_label": "DERIVED_L2_STATE_COMPARISON; no success threshold implied",
        }
        l2_state_comparison[case_id] = row
    ...
    failure_cases = sorted(set(control_failure_cases + first_response_failure_cases), key=lambda case_id: (CASE_L2[case_id], case_id))
    first_guardrail_failure_boundary = CASE_L2[failure_cases[0]] if failure_cases else None
    runs_after_first_guardrail_failure = [case_id for case_id in active_new_runs if first_guardrail_failure_boundary is not None and CASE_L2[case_id] > first_guardrail_failure_boundary]
    protocol_audit = {
        "schema": "bjs400-l2-highside-second-trigger-protocol-audit-v1",
        "status": "FAIL" if runs_after_first_guardrail_failure else "PASS",
        "rule": "stop at the first registered CONTROL or first-response failure boundary",
        "first_guardrail_failure_boundary_L2_pH": first_guardrail_failure_boundary,
        "first_guardrail_failure_cases": [case_id for case_id in failure_cases if CASE_L2[case_id] == first_guardrail_failure_boundary] if first_guardrail_failure_boundary is not None else [],
