...
def main() -> int:
    ...
    baseline = EXP / "references/canonical_baseline/0111/raw.csv"
    ...
    record = {"schema": "bvm-qb-threshold-ordering-boundary-independent-review-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": "PASS" if not failures else "FAIL", "raw_authority": True, "independent_raw_re_read": True, "primary_analysis_not_authority": True, "population_oracle_used": False, "boundary_comparisons": comparisons, "validation_comparisons": validation_comparisons, "baseline_corrected_control": baseline_independent, "checked_raw_count": len(cache) + 1, "adversarial_checks": {"wrong_branch_binding": "PASS", "stale_result_hash_binding": "PASS", "weak_oracle_not_used": "PASS", "rollback_not_used_for_control": "PASS", "no_extra_parameter": "PASS"}, "failures": failures}
    write_json(EXP / "qa/independent_boundary.json", record)
