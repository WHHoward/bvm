...
**M11A 接受 / M11B 重签更新（2026-08-13）**：`JH-20260813-M11A-001` A02 已由 `audits/C02/verdict.yaml` `ACCEPTED`。它封存 M4--M10 的唯一 accepted/superseding 证据、精确 M10 十一项 artifact 清单、M6 同 JJ 控制 raw、M8 六次 fixture-local convergence raw 与六组回归；全局容差仍为 `UNFROZEN`，不构成物理或接口 Gate。M11B-001 的 A01/A02 与 C01 均保留为历史，不得作为最终重建基线；用户要求的统一 object matrix 将以新的 superseding FROZEN 合同实现，覆盖 BVM storage、BVM source/output、published QB、BQ v4、standard DCSFQ、DCSFQ_BVM 与 canonical JTL。它同时是 W5B provenance 的唯一事实层：只有其审计接受后 W5B 才闭环，且不会建立第二套 provenance 记录。M11 仍未完成，候选路线、`INTERFACE_GATE_V1` 与 T1 仍不得启动。
