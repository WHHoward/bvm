...
> [!CAUTION]
> **2026-08-09 单位审计后，本日志中的所有 SFQ/phase 计数及基于 `fast_events` 的判定均需重算。** 特别是 BQ v4 在 110/130/150 µA 已测点的现有 CSV 显示六个输入对应下游约六圈，与约 1:1 传播相容，旧“38 个失控事件/输出级死亡”结论错误；完整 SFQ Gate 仍待重验。非相位的电流、电压和波形数据仍可作为原始观察。当前口径见 `docs/HANDOVER.md`。
...
## 实验 5: K 元件变压器方案 (2026-07-17, 已完成)

> ⚠️ 2026-08-06 更新: 本实验已完成，原日志误标"待进行"。结果从 memory 记录补回。
...
**失效原因**:
1. L_PRI=2pH 在 500GHz 下 Z≈6.3Ω, 与 8×jjmit 负载并联分流严重
2. M=K√(L_PRI×L_SEC)=3.6pH, 在 SFQ 时间尺度 (~30ps) 耦合能量不足
3. L_SEC 串联感抗 Z≈25Ω 限制次级电流
4. 本质: K 元件适合连续正弦波耦合 (MHz-GHz), 不适合 ps 级 SFQ 脉冲瞬时能量传递

**数据**: `/tmp/exp_tx_*.csv`（已丢失）

## 实验 5b: BQ v4 独立验证（Step 1）→ 见上方实验 6
