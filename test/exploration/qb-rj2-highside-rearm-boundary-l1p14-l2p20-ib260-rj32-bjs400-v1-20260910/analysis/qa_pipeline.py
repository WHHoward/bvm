#!/usr/bin/env python3
"""Raw, deck and registered mechanical QA for the RJ2 high-side boundary search."""
...
CASE_RJ2 = {**{run_id: 6.0 for run_id in REUSE_RUNS}, **RUN_TO_RJ2}
ORIGINS = OrderedDict(
    (
...
def deck_diff() -> dict[str, Any]:
    ...
    for run_id in REUSE_RUNS:
        ...
        expected = build_deck(6.0, run_id.rsplit("_", 1)[1])
    ...
    if historical_failures:
        failures.append("historical RJ2=6.0 deck comparability failed: " + ",".join(historical_failures))
    delta = bq_delta()
    if delta["status"] != "PASS":
        failures.append("BQ RJ2 parameterization delta is not exact")
    return {
        "schema": "bjs400-rj2-highside-deck-diff-qa-v1",
...
def main() -> int:
    ...
    second_response_cases = [case_id for case_id in recross_cases if cases[case_id]["SECOND_RESPONSE_AFTER_FIRST_BJ2"].get("status") == "BOUNDED_RESULT"]
    ...
    elif recross_cases and second_response_cases:
        bounded_outcome = "OUTCOME_A_SECOND_MULTI_EVIDENCE_RESPONSE_CANDIDATE"
    elif recross_cases:
        bounded_outcome = "OUTCOME_B_OBSERVED_L1_RECROSSING_WITHOUT_SECOND_COMPLETE_MULTI_EVIDENCE_CANDIDATE"
    else:
