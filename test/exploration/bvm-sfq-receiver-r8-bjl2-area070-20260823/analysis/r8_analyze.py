#!/usr/bin/env python3
"""Independent R8 BJL2 AREA=.70 raw analysis.
...
RMS routing metrics for the single L1 point.
"""
...
RAW = RUN / "raw"
OUT = RUN / "analysis"
R7A = REPO / "test/exploration/bvm-sfq-receiver-r7a-l1-routing-20260823"
CANONICAL = REPO / "test/exploration/bvm-internal-readout-20260819/raw"
RUN_FILE = "run-01.csv"
R7A_RUN_FILE = "run-02.csv"
...
def monotonic_segments(rows, phase_key, voltage_key, window):
    ...
    # BJL2 AREA comparison does not change the segmentation rule.
    half = 5
...
def bjl2_comparison(result, baseline):
    current = result["junctions"]["BJL2"]
    reference = baseline["junctions"]["BJL2"]
    current_segment = current["largest_segment"]
    reference_segment = reference["largest_segment"]
    return {
        "activity_range_gain": current["activity_range_turn"] / reference["activity_range_turn"],
        "largest_segment_gain_abs": abs(current_segment["phase_delta_turns"]) / abs(reference_segment["phase_delta_turns"]),
        "same_jj_area_gain_abs": abs(current_segment["area_turns"]) / abs(reference_segment["area_turns"]),
        "current_excursion_gain": current["activity_current"]["p2p"] / reference["activity_current"]["p2p"],
        "voltage_peak_gain_abs": current["activity_voltage"]["peak_abs"] / reference["activity_voltage"]["peak_abs"],
        "r8_activity_range_turn": current["activity_range_turn"],
        "r7a_activity_range_turn": reference["activity_range_turn"],
        "r8_largest_segment_turn": current_segment["phase_delta_turns"],
        "r7a_largest_segment_turn": reference_segment["phase_delta_turns"],
        "r8_same_jj_area_turn": current_segment["area_turns"],
        "r7a_same_jj_area_turn": reference_segment["area_turns"],
    }
...
def main():
    ...
    r7a_infos = {case: load_case(R7A, case, R7A_RUN_FILE) for case in cases}
    r7a_routing = {
        "read1": routing(r7a_infos["read1"], r7a_infos["logical1-read0-control"]),
        "read0": routing(r7a_infos["read0"], r7a_infos["logical0-read0-control"]),
    }
    bjl2_comparisons = {
        case: bjl2_comparison(results[case], case_result(r7a_infos[case]))
        for case in ["read1", "read0"]
    }
    ...
    output = {
        "analysis": {
            ...
            "R7A_reference_routing": r7a_routing,
            "raw_sha256": {
        ...
        "R7A_cases": {case: case_result(r7a_infos[case]) for case in cases},
        "comparison_to_R7A": bjl2_comparisons,
        ...
        "verdict": "OUTPUT_CLASS_CHANGE_WITHOUT_MEANINGFUL_BJL2_GAIN",
        "local_native_qb_pass": False,
        "static_operating_point_shift": True,
    }
    output_path = OUT / "r8-summary.json"
