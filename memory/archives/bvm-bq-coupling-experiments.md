...
> [!CAUTION]
> **2026-08-09：本文中的 `P()`/SFQ 数和基于 `fast_events` 的失败判定需重算。** 原始文件索引仍有用；BQ v4 ≥110 µA 的下游现有数据实际约为每输入一事件，旧“38 个失控事件/路线排除”错误。当前口径见 `docs/HANDOVER.md`。
...
**修改**: BJL1 36→90µA, BJL2 54→70µA, RJ1→56Ω, RJ2→36Ω, L0→2.5pH (IC 顺序 BJs<BJL2<BJL1)

| 测试 | 结果 |
|------|------|
| S1.2 偏置稳定 | ✅ BJs=0.0, 0 fast_events |
| S1.3 SFQ 注入 | ❌ BJs +12.57 SFQ/脉冲 (过驱动), JTL 收到 0 |
| S1.4 电流扫参 | ❌ BJs 全程 602-829 SFQ 滑移; JTL B1: ≤90µA→0, ≥110µA→38 (无控) |
| S1.5 JTL 计数 | ❌ BVM 水平 (70µA) 收到 0 |
| S1.6 支路诊断 | ⚠️ BJL1 不吞噬 ✅ 但 BJL2 也不触发 ❌ |

**关键对照 (90µA 同测试台)**: v2 Vpk=1035µV vs v4 Vpk=186µV — v4 输出差 5.5×

**冻结根因**:
1. **BJs 输入级**: 无分流电阻 + βc≈5.4 欠阻尼 → 任何过驱动 (1.4×IC) 即持续电压态滑移, 从未干净单翻转
2. **v4 副作用**: IC 顺序修正消除了信号吞噬, 但也消灭了输出传递 → 能量困在 BJs 滑移环路
3. **输出级驱动不足**: v2/v4 都无法驱动 250µA ColdFlux JTL (输出脉冲电流 ~100µA < 阈值)

**意义**: 修改 bq 参数 (IC 顺序) 不能解决根本问题 — 问题在 BJs 输入级物理 (欠阻尼无分流) 和输出级驱动能力。→ Step 3 决策: 迭代 BQ (加分流) / Step 4 备用接口 / 重审 BVM 输出级
