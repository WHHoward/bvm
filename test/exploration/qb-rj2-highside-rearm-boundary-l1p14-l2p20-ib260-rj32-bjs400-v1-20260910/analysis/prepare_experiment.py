#!/usr/bin/env python3
"""Register the fixed-point RJ2 high-side re-arm boundary experiment.

This script performs no physical solve. It creates the six registered RJ2
decks and verifies the exact RJ2=6 ohm historical baseline before execution.
"""
...
AUTH = REPO / "test/exploration/qb-rj2-post-bj2-damping-l1p14-l2p20-ib260-rj32-bjs400-v1-20260910"
...
AUTH_SOURCE_MANIFEST_SHA256 = "bd110a770749641adf7a7a96bb414aca9ea91130a7ccc3b5430d53dbd42dfcb7"
AUTH_PACKAGE_SHA256 = "897a8c2cbf17f9aba4fdc688a82b62080a1398b4afcacbde30db9411bd9dfb09"
AUTH_GIT_HEAD = "c8fc8c5b6483d65e7b2e04e470e81ca07c95fd3f"
...
def write_reuse_manifest() -> dict[str, Any]:
    ...
    authority_package = AUTH / "handoff/qb-rj2-post-bj2-damping-l1p14-l2p20-ib260-rj32-bjs400-v1-20260910_raw_handoff.zip"
    ...
    for run_id, spec in SOURCE_REUSE.items():
        ...
        expected_deck = build_deck(6.0, spec["mask"])
        ...
        if params.get("L1_pH") != 1.4 or params.get("L2_pH") != 2.0 or params.get("IBias_uA") != 260.0 or params.get("RJ1_ohm") != 32.0 or params.get("RJ2_ohm") != 6.0:
            failures.append("fixed parameter mismatch")
        if metadata.get("rj2_ohm") != 6.0:
        ...
        if failures:
            raise RuntimeError(f"RJ2=6ohm exact reuse verification failed for {run_id}: {'; '.join(failures)}")
        references[run_id] = {
            ...
            "parameters": {"L1_pH": 1.4, "L2_pH": 2.0, "IBias_uA": 260.0, "RJ1_ohm": 32.0, "RJ2_ohm": 6.0},
            "topology": "ARRAY",
            "mask": spec["mask"],
            "physical_solve_this_experiment": False,
            "reuse_reason": "exact RJ2=6ohm baseline under fixed L1/L2/IBias/RJ1, BJS400 fixture, stored history, mask semantics, solver/timestep/stop-time and source/archive hashes match",
            "comparability": {
...
def write_source_manifest() -> dict[str, Any]:
    ...
    record = {
        "schema": "bjs400-rj2-highside-source-manifest-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
...
def verify_decks() -> dict[str, Any]:
    ...
    return {
        "schema": "bjs400-rj2-highside-deck-diff-preflight-v1",
...
def preflight_markdown(record: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "",
        f"- RJ2=6ohm reuse comparability: `{record['reuse_comparability']['status']}`.",
...
def main() -> int:
    ...
    record = {
        "schema": "bjs400-rj2-highside-preflight-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "status": "PASS",
        "head": head(),
