#!/usr/bin/env python3
"""Raw, deck and registered mechanical QA for the RJ2 damping sweep."""
...
CASE_RJ2 = {**{run_id: 4.0 for run_id in REUSE_RUNS}, **RUN_TO_RJ2}
ORIGINS = OrderedDict(
    (
...
def deck_diff() -> dict[str, Any]:
    ...
    for run_id in NEW_RUNS:
        ...
        records[run_id] = {"RJ2_ohm": rj2, "mask": mask, "deck_sha256": sha256(deck_path(run_id)), "deck_bytes": deck_path(run_id).stat().st_size}
    if len(normalized) != 1:
    ...
    for run_id in REUSE_RUNS:
        ...
        expected = build_deck(4.0, run_id.rsplit("_", 1)[1])
    ...
    if historical_failures:
        failures.append("historical RJ2=4.0 deck comparability failed: " + ",".join(historical_failures))
    delta = bq_delta()
    if delta["status"] != "PASS":
        failures.append("BQ RJ2 parameterization delta is not exact")
    return {
        "schema": "bjs400-rj2-damping-deck-diff-qa-v1",
...
def main() -> int:
    ...
    for case_id in ALL_CASES:
        ...
        try:
            ...
            post = post_first_bj2_rearm(trace, origin_records["FINAL_ORIGIN"], FIXED["L2_pH"])
            diff = differential_voltage_impulse(trace, origin_records["FINAL_ORIGIN"], FIXED["L2_pH"])
    ...
    raw_qa = {"schema": "bjs400-rj2-damping-raw-qa-v1", "status": "PASS" if raw_ok else "FAIL", "artifact_validity": "VALID" if raw_ok else "INVALID", "experiment_id": EXP.name, "created_at_local": now(), "head": relation["head"], "execution_status": "PASS" if execution_ok else "FAIL", "new_physical_solve_count": 6, "reused_physical_case_count": 2, "exact_logical_case_count": 8, "unauthorized_extra_solves": 0, "cases": raw_records, "pre_analysis_sha256": pre_hashes, "post_analysis_sha256": post_hashes, "raw_unchanged_pre_to_post": pre_hashes == post_hashes, "common_time_grid": len(grid_hashes) == 1, "stored_grid_hashes": sorted(grid_hashes), "required_probe_missing_is_not_fabricated": True, "raw_files_modified": 0, "scientific_analysis_performed": False, "failures": raw_failures}
    decks = deck_diff()
