...
COMPARISON_NAMES = ["ALL_SELECTED_MASKS.html", "WEIGHT_2_POSITION.html", "WEIGHT_3_POSITION.html", "DOWNSTREAM_TERMINAL_POPULATION.html"]
...
def main() -> int:
    ...
    if len(standalone) != 42 or len(comparisons) != 4:
    ...
    expected_standalone = {
        f"{experiment_prefix}/visualization/plots/runs/{run_id}/{name}" for run_id in RUN_IDS for name in STANDALONE_NAMES
    }
    ...
    expected_comparison = {f"{experiment_prefix}/visualization/plots/comparison/{name}" for name in COMPARISON_NAMES}
    ...
    actual_html = {
        ...
        for path in (EXP / "visualization/plots").rglob("*.html")
    }
    expected_html = {path.removeprefix(experiment_prefix + "/") for path in expected_standalone | expected_comparison}
    if actual_html != expected_html:
        failures.append("visualization/plots contains extra/missing HTML artifacts")
    if list((EXP / "visualization/plots").rglob("*.csv")):
        failures.append("visualization plot directory contains a duplicate CSV")
    if (EXP / "visualization/derived").exists():
        failures.append("standalone/comparison derived visualization directory exists")
    if any("window" in path.name.casefold() or "category" in path.parts for path in (EXP / "visualization/plots").rglob("*")):
        failures.append("focused/category plot artifact detected")
    qa = {
        "schema": "bjs400-selected-mask-flat-visualization-qa-v1",
