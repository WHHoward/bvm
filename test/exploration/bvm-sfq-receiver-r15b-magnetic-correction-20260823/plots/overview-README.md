# bvm-sfq-receiver-r15b-magnetic-correction-20260823 overview
...
- phase: `P(B_OUT|XAFQ)` → rad/2π turns
- phase: `P(B3|XDCS)` → rad/2π turns
- phase: `P(B2|XDCS)` → rad/2π turns
- phase: `P(B1|XDCS)` → rad/2π turns
- voltage: `V(B_OUT|XAFQ)` → µV
- voltage: `V(SL1)` → µV
- current: `I(L_SL|XBVM1)` → µA
- current: `I(I_BL1)` → µA
- current: `I(I_WL1)` → µA
- current: `I(L1|XDCS)` → µA
