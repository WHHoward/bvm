#!/usr/bin/env python3
"""Mechanical QA for the targeted L1/L3/BJ2 full closed-loop experiment."""
...
MATRIX = EXP / "screening/CLOSURE_MATRIX.json"
...
POINT_ID = "TARGET_L1_1p2_L3_2_BJ2_2p2"
SCREENING_MASKS = ("0011", "0111")
ALL_MASKS = SCREENING_MASKS
...
ORIGINS = {
    ...
    "l1p12_l3p13_bj2p20/0011": REPO / "test/exploration/bvm-full-closed-loop-qb-parameter-screening-v1-20260911/runs/SCREEN_L1_pH_1p2_0011",
    "l1p12_l3p13_bj2p20/0111": REPO / "test/exploration/bvm-full-closed-loop-qb-parameter-screening-v1-20260911/runs/SCREEN_L1_pH_1p2_0111",
    "l3p20_bj2p22/0011": REPO / "test/exploration/bvm-qb-l3-bj2-targeted-combination-v1-20260911/runs/TARGET_L3_2_BJ2_2p2_0011",
    "l3p20_bj2p22/0111": REPO / "test/exploration/bvm-qb-l3-bj2-targeted-combination-v1-20260911/runs/TARGET_L3_2_BJ2_2p2_0111",
}
...
def deck_check(path: Path, point: dict[str, Any], mask: str) -> dict[str, Any]:
    ...
    for expected in (".param L1_VALUE=1.2p", ".param IB_VALUE=260u", ".param RJ1_VALUE=32", ".param RJ2_VALUE=12"):
        if expected not in text:
            failures.append(f"missing frozen active parameter: {expected}")
    variant = EXP / "inputs/bq_variants/TARGET_L1_1p2_L3_2_BJ2_2p2_L1_pH1p2_L3_pH2_BJ2_area2p2.cir"
...
def main() -> int:
    ...
    result_path = EXP / "screening/results/TARGET_L1_1p2_L3_2_BJ2_2p2.json"
    results: dict[str, Any] = {}
    if result_path.is_file():
        result = json.loads(result_path.read_text(encoding="utf-8"))
        results[POINT_ID] = {"path": rel(result_path), "sha256": sha256(result_path), "candidate_class": result.get("candidate_class"), "candidate_label": result.get("candidate_label"), "analysis_version": result.get("analysis_version")}
    ...
    if len(run_dirs) != 2 or execution.get("screening_new_physical_solve_count") != 2:
        failures.append("solve budget or screening count mismatch")
    if execution.get("validation_new_physical_solve_count") != 0 or state.get("validation_cases"):
        failures.append("validation solve was run without separate scientific-review authorization")
    ...
    write_json(EXP / "qa/raw_qa.json", {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-raw-qa-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": raw_status, "artifact_validity": "VALID" if raw_status == "PASS" else "ARTIFACT_INVALID", "required_probe_count": len(REQUIRED), "references": reference_records, "runs": runs, "raw_files_modified": 0, "failures": failures})
    write_json(EXP / "qa/deck_diff_qa.json", {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-deck-qa-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": deck_status, "decks": decks, "target_only_changes": {"L1_pH": 1.2, "L3_pH": 2.0, "BJ2_area": 2.2}, "failures": failures})
    write_json(EXP / "qa/provenance.json", {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-provenance-v1", "experiment_id": EXP.name, "created_at_local": now(), "repository_head_at_qa": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=REPO, text=True).strip(), "preflight": {"path": rel(EXP / "analysis/preflight.json"), "sha256": sha256(EXP / "analysis/preflight.json")}, "source_manifest": {"path": rel(EXP / "SOURCE_MANIFEST.json"), "sha256": sha256(EXP / "SOURCE_MANIFEST.json")}, "matrix": {"path": rel(MATRIX), "sha256": sha256(MATRIX)}, "solver": {"path": rel(SOLVER), "sha256": sha256(SOLVER), "version": subprocess.check_output([str(SOLVER), "--version"], cwd=REPO, text=True)}, "runs": {name: {"raw_sha256": item.get("sha256"), "bytes": item.get("bytes")} for name, item in runs.items()}, "references": {key: {"raw_sha256": value["raw.csv"].get("sha256"), "bytes": value["raw.csv"].get("bytes")} for key, value in reference_records.items()}, "results": results, "reference_roles": {"R0_canonical": "L1=1.4 L3=1.3 BJ2=2.0", "R1_l1_rescue": "L1=1.2 L3=1.3 BJ2=2.0", "R2_l3_bj2_barrier": "L1=1.4 L3=2.0 BJ2=2.2"}, "target": {"L1_pH": 1.2, "L3_pH": 2.0, "BJ2_area": 2.2}, "raw_authority": True, "no_extra_parameter": True, "no_timestep_change": True})
    write_json(EXP / "qa/transformation_registry.json", {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-transformation-registry-v1", "experiment_id": EXP.name, "created_at_local": now(), "raw_immutable": True, "phase": "independent continuous unwrap of raw radians then rad/(2*pi) navigation only", "integration": "actual-grid trapezoid without interpolation", "control": "complete QBOUT->JTL1..6 chain only in registered 70-110 ps windows; FINAL/Tail rollback excluded", "source_feedback": "actual-grid I(B_JSL8) signed areas", "visualization": "raw-direct standalone and temporary exact-grid focused comparison deleted after render", "forbidden": ["replay", "passive capture", "source modification", "source scaling", "parameter sweep", "third parameter", "timestep change", "validation without scientific review"], "scientific_review_status": "AWAITING_SCIENTIFIC_REVIEW"})
    mechanical = "PASS" if not failures and raw_status == "PASS" and deck_status == "PASS" else "FAIL"
    write_json(EXP / "mechanical_summary.json", {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-mechanical-summary-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": mechanical, "raw_qa_status": raw_status, "deck_qa_status": deck_status, "execution_status": execution.get("status"), "scientific_interpretation_performed": False, "scientific_review_status": "AWAITING_SCIENTIFIC_REVIEW", "raw_files_modified": 0, "failures": failures})
