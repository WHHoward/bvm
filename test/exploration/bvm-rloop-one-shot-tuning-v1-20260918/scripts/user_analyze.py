...
def main() -> int:
    ...
    manifest_out = {"schema": "bvm-rloop-user-case-analysis-manifest-v1", "case_id": manifest["case_id"], "windows": manifest["windows"], "raw_qa": str((qa_dir / "raw_qa.json").relative_to(SERIES)).replace("\\", "/"), "case_metrics": str((analysis_dir / "case_metrics.json").relative_to(SERIES)).replace("\\", "/"), "window_metrics": str((analysis_dir / "per_signal_window_metrics.csv").relative_to(SERIES)).replace("\\", "/"), "population_metrics": str((analysis_dir / "population_metrics.csv").relative_to(SERIES)).replace("\\", "/"), "scientific_interpretation_performed": False, "automatic_follow_up": False}
    ...
    write_json(case_root / "result.json", {"schema": "bvm-rloop-user-case-result-v1", "case_id": manifest["case_id"], "status": status, "artifact_status": "VALID" if raw_qa["status"] == "PASS" else "INVALID", "physical_solve_count": manifest["physical_solve_count"], "scientific_interpretation_performed": False, "automatic_follow_up": False})
