#!/usr/bin/env python3
"""Register the fixed-point RJ2 post-BJ2 damping experiment.

This script performs no physical solve. It creates the six registered RJ2
decks and verifies the exact RJ2=4 ohm historical baseline before execution.
"""
...
AUTH_GIT_HEAD = "9e1401021c4247017f5459d2c6c0cd67dcac1c30"
...
RUN_TO_RJ2 = {
    run_id: l2
    for l2 in RJ2_VALUES
    for run_id in (f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P{int(l2)}_0001", f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P{int(l2)}_0011")
}
...
def write_reuse_manifest() -> dict[str, Any]:
    ...
    for run_id, spec in SOURCE_REUSE.items():
        ...
        expected_deck = build_deck(4.0, spec["mask"])
        ...
        if failures:
            raise RuntimeError(f"RJ2=4ohm exact reuse verification failed for {run_id}: {'; '.join(failures)}")
        references[run_id] = {
            ...
            "parameters": {"L1_pH": 1.4, "L2_pH": 2.0, "IBias_uA": 260.0, "RJ1_ohm": 32.0, "RJ2_ohm": 4.0},
            "topology": "ARRAY",
            "mask": spec["mask"],
            "physical_solve_this_experiment": False,
            "reuse_reason": "exact RJ2=4ohm baseline under fixed L1/L2/IBias/RJ1, BJS400 fixture, stored history, mask semantics, solver/timestep/stop-time and source/archive hashes match",
            "comparability": {
    ...
    record = {
        "schema": "bjs400-rj2-damping-reuse-manifest-v1",
...
def write_source_manifest() -> dict[str, Any]:
    ...
    record = {
        "schema": "bjs400-rj2-damping-source-manifest-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
...
def verify_decks() -> dict[str, Any]:
    ...
    return {
        "schema": "bjs400-rj2-damping-deck-diff-preflight-v1",
...
def preflight_markdown(record: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "",
        f"- RJ2=4ohm reuse comparability: `{record['reuse_comparability']['status']}`.",
...
def main() -> int:
    ...
    record = {
        "schema": "bjs400-rj2-damping-preflight-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "status": "PASS",
        "head": head(),
