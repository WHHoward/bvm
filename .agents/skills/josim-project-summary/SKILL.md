...
## 研究历程记录（2026-08-09 起强制，用户要求）

`memory/history/research-history.md` 是研究叙事的权威时间线（为什么做、做了什么、结论如何演变）。出现以下**任何一类**变化时，必须追加一个带日期的时间线条目，并标注结论状态（✅ 仍有效 / ⚠️ 已修正或失效 / 🔄 进行中）：

1. 阶段级里程碑：路线转向、候选元件创建、Phase 开始/完成；
2. 结论级变化：审计发现、事故、Gate 判定、撤回或修正结论；
3. 基础设施变化：工作流协议、工具、跨模型协作机制；
4. 重大交付：合同化任务交付、论文材料更新。

条目格式：**日期 + 做了什么 + 结果/产物路径 + 认知状态**。已失效结论必须写"被什么替代"，不能只删不改。更新后同步 frontmatter `last_updated` 与 `MEMORY.md` 索引。
