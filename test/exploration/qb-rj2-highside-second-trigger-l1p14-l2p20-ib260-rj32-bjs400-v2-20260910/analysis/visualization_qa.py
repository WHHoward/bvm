#!/usr/bin/env python3
"""Visualization QA for the staged RJ2 second-trigger evidence."""
...
COMPARISON_NAMES = ("RJ2_SECOND_TRIGGER_0001_COMPARE.html", "RJ2_SECOND_TRIGGER_0011_COMPARE.html")
...
def check_entry(entry: dict[str, Any], failures: list[str]) -> None:
    ...
    if entry.get("stage") == "standalone":
        ...
        if not str(input_path).startswith("/tmp/rj2-second-trigger-") or input_path.exists():
...
def main() -> int:
    ...
    if {entry.get("output_path") for entry in comparisons} != expected_comparison:
        failures.append("comparison paths are not exactly RJ2_SECOND_TRIGGER_0001/RJ2_SECOND_TRIGGER_0011")
    ...
    qa = {
        "schema": "bjs400-rj2-highside-second-trigger-visualization-qa-v2",
