#!/usr/bin/env python3
"""Freeze exact passive JSL8 current snapshots and generate two replay decks."""
...
def replay_deck(case: str, source_path: Path, rows: list[dict[str, str]]) -> str:
    return "\n".join(
        [
            f"* GENERATED REPLAY_{case.upper()} DECK: exact passive I(JSL8) current replay",
...
def snapshot_record(path: Path, source_signal: str, rows: list[dict[str, str]]) -> dict[str, object]:
    return {
...
def main() -> int:
    ...
    for passive, replay in (("single_passive", "replay_single"), ("array_passive", "replay_array")):
        raw_path = EXP / "runs" / passive / "raw.csv"
        if not raw_path.is_file():
            raise RuntimeError(f"missing passive raw: {raw_path}")
        _, rows = read_source(raw_path)
        snapshot_path = EXP / "data" / f"{passive}_JSL8_source.csv"
        ...
        source_records[passive] = {
            ...
            "snapshot": snapshot_record(snapshot_path, SOURCE_SIGNAL, rows),
            "orientation": {
        ...
        deck_path = EXP / "runs" / replay / "deck.cir"
        deck = replay_deck(passive, snapshot_path, rows)
    ...
    manifest = {
        "schema": "jm2-passive-capture-replay-source-snapshot-manifest-v1",
