#!/usr/bin/env python3
"""Build and verify the immutable full-grid evidence ZIP."""
...
def main() -> int:
    ...
    if execution.get("status") != "PASS" or execution.get("exact_new_physical_solve_count") != 10 or execution.get("reused_physical_case_count") != 14 or execution.get("unauthorized_extra_solves") != 0:
        raise RuntimeError("execution QA is not exact 10-new/14-reuse PASS")
    if viz_qa.get("status") != "PASS" or viz_qa.get("standalone_html_count") != 30 or viz_qa.get("comparison_html_count") != 2:
        raise RuntimeError("visualization QA is not exact 30+2 PASS")
    ...
    package = build_package(EXP, package_path=PACKAGE_RELATIVE, physics_solve_count=10, scientific_analysis_performed=False, include_plots=False)
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        if not any(name.startswith("references/reused/") for name in name_set):
            failures.append("historical references were not packaged")
    ...
    package_qa.update({
        ...
        "new_physical_solve_count": 10,
        "reused_physical_case_count": 14,
        "exact_logical_case_count": 24,
