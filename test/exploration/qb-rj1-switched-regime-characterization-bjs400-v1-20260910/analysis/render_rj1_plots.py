...
def render_comparison(entries: list[dict[str, object]], name: str, case_ids: list[str]) -> None:
    ...
    temp_path = Path(tempfile.mkstemp(prefix="rj1-switched-regime-", suffix=".csv", dir="/tmp")[1])
    try:
...
def write_run_summaries() -> list[Path]:
    ...
    for run_id in NEW_RUNS:
        ...
        lines = [
            f"# {run_id} — RJ1 switched-regime evidence",
            "",
...
def main() -> int:
    ...
    manifest = {
        "schema": "rj1-switched-regime-flat-visualization-manifest-v1",
