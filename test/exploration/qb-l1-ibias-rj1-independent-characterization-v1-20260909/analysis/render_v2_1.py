...
RUNS = {
    "L1_1P2": ("runs/L1_1P2", 12.0, 1.2, 250.0),
    "L1_1P4": ("runs/L1_1P4", 12.0, 1.4, 250.0),
    "L1_1P6": ("runs/L1_1P6", 12.0, 1.6, 250.0),
    "L1_1P8": ("runs/L1_1P8", 12.0, 1.8, 250.0),
    "L1_1P9": ("references/reused/L1_1P9", 12.0, 1.9, 250.0),
    "L1_NOMINAL": ("references/reused/NOMINAL", 12.0, 2.0, 250.0),
    "L1_2P1": ("references/reused/L1_2P1", 12.0, 2.1, 250.0),
    "IB_230": ("runs/IB_230", 12.0, 2.0, 230.0),
    "IB_240": ("runs/IB_240", 12.0, 2.0, 240.0),
    "IB_NOMINAL": ("references/reused/NOMINAL", 12.0, 2.0, 250.0),
    "IB_260": ("runs/IB_260", 12.0, 2.0, 260.0),
    "IB_270": ("runs/IB_270", 12.0, 2.0, 270.0),
    "RJ1_8": ("runs/RJ1_8", 8.0, 2.0, 250.0),
    "RJ1_10": ("runs/RJ1_10", 10.0, 2.0, 250.0),
    "RJ1_NOMINAL": ("references/reused/NOMINAL", 12.0, 2.0, 250.0),
    "RJ1_12P5": ("references/reused/RJ1_12P5", 12.5, 2.0, 250.0),
    "RJ1_13": ("references/reused/RJ1_13", 13.0, 2.0, 250.0),
    "RJ1_14": ("runs/RJ1_14", 14.0, 2.0, 250.0),
    "RJ1_16": ("runs/RJ1_16", 16.0, 2.0, 250.0),
}
FAMILIES = {
...
def raw_path(run_id: str) -> Path:
    return EXP / RUNS[run_id][0] / "raw.csv"
...
def ensure_v21_raw_reference() -> dict[str, object]:
    ...
    if path.is_file():
        return json.loads(path.read_text(encoding="utf-8"))
    record = {
        "schema": "qb-l1-ibias-rj1-independent-v2-1-raw-reference-v1",
        "version": "V2.1",
        "created_at_local": datetime.now(timezone.utc).astimezone().isoformat(timespec="seconds"),
        "experiment_id": EXP.name,
        "source_manifest": {"path": str(EXP / "SOURCE_MANIFEST.json"), "sha256": sha256(EXP / "SOURCE_MANIFEST.json")},
        "reused_reference_manifest": {"path": str(EXP / "REUSED_REFERENCE_MANIFEST.json"), "sha256": sha256(EXP / "REUSED_REFERENCE_MANIFEST.json")},
        "raw_authority": {run_id: {"path": str(raw_path(run_id)), "sha256_at_v21_start": sha256(raw_path(run_id)), "physical_solve_this_experiment": run_id in {item[0] for item in RUNS.values() if item[0].startswith("runs/")}} for run_id in RUNS},
...
def render_v21(stage: str) -> int:
    ...
    if stage == "comparison":
        ...
        manifest = {
            ...
            "reused_reference_manifest": reference["reused_reference_manifest"],
...
def write_v21_run_summaries() -> None:
    ...
    for run_id, spec in RUNS.items():
        ...
        for category, windows in V21_STANDALONE_WINDOWS.items():
            ...
            links = [f"[{name}]({base}/standalone/{directory}/{category}/{name}.html)" for name in windows]
            lines.append(" · ".join(links))
            lines.append("")
        lines.extend(["## Boundaries", "", "Every category uses INPUT -> INTERNAL -> OUTPUT ordering.", "", "## Unknowns", "", "Missing emitted probe columns remain UNKNOWN; no event or mechanism classification is generated.", ""])
        text = "\n".join(lines)
