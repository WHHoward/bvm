...
REFERENCE_PATHS = {
    mask: EXP / "references/rj2p12" / f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P12_{mask}" / "raw.csv"
    for mask in ("0001", "0011", "0111", "1111")
}
...
def main() -> int:
    ...
    cases = list(RUN_IDS)
    paths = [raw_path(case) for case in cases]
    specs = signal_specs("ARRAY")
    render_comparison(
        manifest_entries,
        "POPULATION_CHAIN_RJ2P11.html",
        cases,
        paths,
        specs["SIGNAL_PATH"],
        EXP / "plots/comparison/POPULATION_CHAIN_RJ2P11.html",
    )
    for mask in ("0011", "0111", "1111"):
        new_case = next(run_id for run_id in RUN_IDS if run_id.endswith(f"_{mask}"))
        render_comparison(
            manifest_entries,
            f"RJ2P11_VS_RJ2P12_{mask}.html",
            [f"RJ2P11_{mask}", f"RJ2P12_{mask}"],
            [raw_path(new_case), REFERENCE_PATHS[mask]],
            matched_comparison_labels(),
            EXP / "plots/comparison" / f"RJ2P11_VS_RJ2P12_{mask}.html",
        )
    ...
    manifest = {
        "schema": "bjs400-rj2p11-midpoint-flat-visualization-manifest-v1",
        ...
        "focused_windows": [],
        "standalone_input_policy": "RJ2=11 raw.csv direct; no standalone derived CSV",
        ...
        "no_extra_masks": True,
        "rj2p12_comparison_reference_policy": "immutable reference raw only",
        "status": "PASS" if len(standalone) == 24 and len(comparisons) == 4 else "FAIL",
    }
