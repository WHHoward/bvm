...
def head_relation(preflight_head: str) -> dict[str, Any]:
    ...
    if status == "PASS":
        return {"status": status, "head": head, "preflight_head": preflight_head, "relation": "PREFLIGHT_ONLY_SEAL_COMMIT", "changed_paths": sorted(changed), "commit_distance": distance}
    ancestor = subprocess.run(["git", "merge-base", "--is-ancestor", preflight_head, head], cwd=REPO, check=False).returncode == 0
    # This experiment is being re-analyzed after later independent experiment
    # commits landed on master.  The raw QA below remains the authority for
    # immutability; the HEAD relation records that this is post-handoff
    # analysis rather than pretending it is a preflight seal commit.
    post_handoff_analysis = ancestor and bool(changed)
    return {"status": "PASS" if post_handoff_analysis else "FAIL", "head": head, "preflight_head": preflight_head, "relation": "POST_HANDOFF_READ_ONLY_ANALYSIS" if post_handoff_analysis else "UNEXPECTED", "changed_paths": sorted(changed), "commit_distance": distance, "raw_immutability_rechecked": post_handoff_analysis}
...
def second_trigger_analysis(trace: Any, final_record: dict[str, Any]) -> dict[str, Any]:
    ...
    legacy_progression = post_anchor_progression(trace, final_record, start_index=second_start_index)
    repaired_oracle = repaired_second_response_oracle(trace)
    return {
        "status": "DERIVED",
        ...
        "second_complete_multi_evidence_candidate": repaired_oracle,
        "legacy_strongest_l1_segment_oracle": legacy_progression,
        "legacy_oracle_false_negative_detected": legacy_progression.get("status") != "BOUNDED_RESULT" and repaired_oracle.get("status") == "BOUNDED_RESULT",
        "second_progression_anchor_type": "strongest_later_positive_L1_segment_start" if second_start_index is not None else "UNKNOWN",
        "second_trigger_gap_reduced_but_not_closed_candidate": bool(first and strongest and repaired_oracle.get("status") != "BOUNDED_RESULT" and second_max > first_max),
...
def main() -> int:
    ...
    legacy_false_negative_cases = [case_id for case_id in cases if cases[case_id]["SECOND_TRIGGER_ANALYSIS"].get("legacy_oracle_false_negative_detected") is True]
    single_response_0001_cases = [case_id for case_id in cases if cases[case_id]["mask"] == "0001" and cases[case_id]["SECOND_TRIGGER_ANALYSIS"].get("second_complete_multi_evidence_candidate", {}).get("single_response_status") == "BOUNDED_RESULT"]
    candidate_population_cases: list[dict[str, Any]] = []
    for case_id in second_response_cases:
        if CASE_RJ2[case_id] != 12.0 or cases[case_id]["mask"] != "0011":
            continue
        paired_0001 = next((candidate for candidate in cases if CASE_RJ2[candidate] == 12.0 and cases[candidate]["mask"] == "0001"), None)
        if paired_0001 is not None and paired_0001 in single_response_0001_cases and cases[paired_0001]["CONTROL_HARD_GUARDRAIL"]["status"] == "CLEAN" and cases[case_id]["CONTROL_HARD_GUARDRAIL"]["status"] == "CLEAN":
            candidate_population_cases.append({"case_id": case_id, "paired_0001_case_id": paired_0001, "label": "CANDIDATE_POPULATION_COUNT_WORKING_POINT", "bounded_only": True, "not_final_sfq_proof": True})
    ...
    result = {"status": "PASS" if raw_ok and decks["status"] == "PASS" and execution_ok and relation["status"] == "PASS" and mechanical_status == "PASS" else "FAIL", "execution_status": "PASS" if execution_ok else "FAIL", "raw_status": raw_qa["status"], "deck_status": decks["status"], "mechanical_summary_status": mechanical_status, "protocol_audit_status": protocol_audit["status"], "bounded_outcome": bounded_outcome, "observed_l1_recrossing_cases": recross_cases, "second_response_candidate_cases": second_response_cases, "second_trigger_gap_reduced_cases": gap_reduced_cases, "legacy_oracle_false_negative_cases": legacy_false_negative_cases, "candidate_population_count_working_point_cases": candidate_population_cases, "control_failure_cases": control_failure_cases, "first_response_failure_cases": first_response_failure_cases, "head_relation": relation["relation"], "logical_case_count": len(ALL_CASES), "raw_failures": raw_failures, "scientific_analysis_performed": False}
