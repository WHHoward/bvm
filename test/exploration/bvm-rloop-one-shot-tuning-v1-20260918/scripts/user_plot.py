...
def main() -> int:
    ...
    base = review.parent
    body = ["<!doctype html><html><head><meta charset='utf-8'><title>" + html.escape(case_root.name) + "</title><style>body{font-family:Arial,sans-serif;background:#111;color:#eee;margin:20px;line-height:1.45}a{color:#8ecbff}section{border:1px solid #444;border-radius:6px;margin:18px 0;padding:14px}table{border-collapse:collapse;width:100%;font-size:12px}th,td{border:1px solid #555;padding:5px}th{background:#292929}code{color:#ffd580}.svgbox{overflow:auto;margin:8px 0 16px}small{color:#aaa}</style></head><body>", f"<h1>{html.escape(case_root.name)}</h1>", f"<p><b>ARRAY_SIZE</b>={html.escape(str(params.get('ARRAY_SIZE', 4)))} · <b>NAME</b>={html.escape(params['NAME'])} · <b>MODE</b>={html.escape(params['MODE'])} · <b>MASKS</b>={', '.join(params['MASKS'])}</p>", f"<p><b>Circuit changes</b><br>{circuit_changes}<br><br><b>Stimulus changes</b><br>{stimulus_changes}</p>"]
