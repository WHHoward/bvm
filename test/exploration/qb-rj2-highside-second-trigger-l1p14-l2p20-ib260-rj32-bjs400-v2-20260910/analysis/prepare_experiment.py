#!/usr/bin/env python3
"""Register the fixed-point RJ2 high-side second-trigger experiment.

This script performs no physical solve. It creates the six registered RJ2
decks and verifies the exact RJ2=10 ohm historical baseline before execution.
"""
...
FIXED = {"L1_pH": 1.4, "L2_pH": 2.0, "IBias_uA": 260.0, "RJ1_ohm": 32.0}
RJ2_VALUES = (12.0, 14.0, 16.0)
MASKS = ("0001", "0011")
CONTROL_KINDS = ("WL", "BL", "SE")
NEW_RUNS = tuple(
    f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P{int(rj2)}_{mask}"
    for rj2 in RJ2_VALUES
    for mask in MASKS
)
...
RUN_TO_RJ2 = {
    run_id: rj2
    for rj2 in RJ2_VALUES
    for run_id in (f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P{int(rj2)}_0001", f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P{int(rj2)}_0011")
}
...
def build_deck(rj2: float, mask: str) -> str:
    ...
    new_bq_include = ".include ../../inputs/BQ_parameterized_bjs400_rj2.cir"
...
def normalized_deck(text: str) -> tuple[str, ...]:
    ...
    for line in text.splitlines():
        if line.startswith(".param L1_VALUE=") or line.startswith(".param IB_VALUE=") or line.startswith(".param RJ1_VALUE=") or line.startswith(".param RJ2_VALUE="):
            continue
        line = line.replace("../../inputs/BQ_parameterized_bjs400_rj2.cir", "../../inputs/BQ_parameterized_bjs400.cir")
...
def write_reuse_manifest() -> dict[str, Any]:
    ...
    if not AUTH.is_dir() or sha256(AUTH / "SOURCE_MANIFEST.json") != AUTH_SOURCE_MANIFEST_SHA256:
        raise RuntimeError("RJ2 high-side source manifest is missing or changed")
    ...
    if not authority_package.is_file() or sha256(authority_package) != AUTH_PACKAGE_SHA256:
        raise RuntimeError("RJ2 high-side source package is missing or changed")
    ...
    for run_id, spec in SOURCE_REUSE.items():
        ...
        expected_deck = build_deck(10.0, spec["mask"])
        ...
        if failures:
            raise RuntimeError(f"RJ2=10ohm exact reuse verification failed for {run_id}: {'; '.join(failures)}")
        references[run_id] = {
            ...
            "reuse_reason": "exact RJ2=10ohm baseline under fixed L1/L2/IBias/RJ1, BJS400 fixture, stored history, mask semantics, solver/timestep/stop-time and source/archive hashes match",
            "comparability": {
    ...
    record = {
        "schema": "bjs400-rj2-highside-second-trigger-reuse-manifest-v2",
...
def write_source_manifest() -> dict[str, Any]:
    ...
    for name, role in (
        ...
        ("BQ_parameterized_bjs400_rj2.cir", "derived BJS400 QB include with RJ2 parameter exposure"),
    ):
    ...
    record = {
        "schema": "bjs400-rj2-highside-second-trigger-source-manifest-v2",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        ...
        "rj2_values_new_ohm": list(RJ2_VALUES),
        "rj2_baseline_reused_ohm": 10.0,
        "rj2_parameterization_delta": {"authority_line": "RJ2 4 0 4", "derived_line": "RJ2 4 0 {RJ2_VALUE}", "derived_source_sha256": sha256(BQ_RJ2)},
...
def generate_decks() -> dict[str, Any]:
    ...
    for run_id in NEW_RUNS:
        ...
        write_once(deck, build_deck(RUN_TO_RJ2[run_id], RUN_TO_MASK[run_id]))
        records[run_id] = {"RJ2_ohm": RUN_TO_RJ2[run_id], "mask": RUN_TO_MASK[run_id], "parameters": {**FIXED, "RJ2_ohm": RUN_TO_RJ2[run_id]}, "path": str(deck.relative_to(REPO)), "sha256": sha256(deck), "bytes": deck.stat().st_size}
    return records
...
def verify_decks() -> dict[str, Any]:
    ...
    records: dict[str, Any] = {}
    bq = BQ_RJ2.read_text(encoding="utf-8")
    ...
    if "RJ2 4 0 {RJ2_VALUE}" not in bq or "RJ2 4 0 4" in bq:
        failures.append("RJ2 parameterized BQ source is not active")
    if "RJ2 4 0 4" not in base_bq:
        failures.append("unmodified BJS400 authority RJ2 line is missing")
    for run_id in NEW_RUNS:
        deck = EXP / "runs" / run_id / "deck.cir"
        ...
        records[run_id] = {"RJ2_ohm": rj2, "mask": mask, "deck_sha256": sha256(deck), "deck_bytes": deck.stat().st_size}
    if len(normalized) != 1:
        failures.append("new decks differ outside RJ2 and final-read controls")
    if len(pre_read) != 1:
    ...
    return {
        "schema": "bjs400-rj2-highside-second-trigger-deck-diff-preflight-v2",
...
def preflight_markdown(record: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "",
        f"- RJ2=10ohm reuse comparability: `{record['reuse_comparability']['status']}`.",
...
def main() -> int:
    ...
    record = {
        "schema": "bjs400-rj2-highside-second-trigger-preflight-v2",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "status": "PASS",
        "head": head(),
        ...
        "early_stop_rule": "after each completed RJ2 pair require a stage decision CONTINUE; stop on control/0001/second-response guardrail failure",
    }
