...
def package(attempt: str) -> dict[str, Any]:
    ...
    for path in sorted(SERIES.rglob("*")):
        relative = path.relative_to(SERIES).as_posix() if path.is_file() else ""
        if not path.is_file() or path.name in {"delivery_manifest.json"} or path.suffix == ".tmp" or "__pycache__" in path.parts or path.name == "PACKAGE_QA.json" or (relative.startswith("visualization/") and "/groups/" in relative and path.suffix == ".html"):
            continue
