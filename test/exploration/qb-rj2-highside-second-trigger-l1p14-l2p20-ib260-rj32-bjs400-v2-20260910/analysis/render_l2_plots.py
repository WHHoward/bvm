...
def render_comparison(entries: list[dict[str, object]], name: str, case_ids: list[str]) -> None:
    ...
    temp_path = Path(tempfile.mkstemp(prefix="rj2-second-trigger-", suffix=".csv", dir="/tmp")[1])
    try:
...
def write_run_summaries() -> list[Path]:
    ...
    for run_id in NEW_RUNS:
        ...
        lines = [
            f"# {run_id} — RJ2 second-trigger evidence",
            "",
...
def main() -> int:
    ...
    render_comparison(entries, "RJ2_SECOND_TRIGGER_0001_COMPARE", COMPARISON_CASES["0001"])
    render_comparison(entries, "RJ2_SECOND_TRIGGER_0011_COMPARE", COMPARISON_CASES["0011"])
    ...
    manifest = {
        ...
        "rj2_values_new_ohm": [12.0, 14.0, 16.0],
