#!/usr/bin/env python3
"""Build and verify the L3 high-side evidence ZIP."""
...
PACKAGE_RELATIVE = "handoff/bvm-qb-l3-highside-selective-window-v1-20260911_raw_handoff.zip"
...
def main() -> int:
    clean_cache()
    names = ("raw_qa.json", "deck_diff_qa.json", "provenance.json", "execution_summary.json", "transformation_registry.json", "independent_l3.json", "visualization_qa.json")
    qa = {name: json.loads((EXP / "qa" / name).read_text(encoding="utf-8")) for name in names}
    state = json.loads((EXP / "screening/l3_state.json").read_text(encoding="utf-8"))
    mechanical = json.loads((EXP / "mechanical_summary.json").read_text(encoding="utf-8"))
    failures = []
    for name in ("raw_qa.json", "deck_diff_qa.json", "execution_summary.json", "independent_l3.json", "visualization_qa.json"):
    ...
    if failures:
        raise RuntimeError("cannot package L3 evidence: " + "; ".join(failures))
    ...
    required = {"experiment.yaml", "PREFLIGHT.md", "SOURCE_MANIFEST.json", "HANDOFF_README.md", "RESULT_BRIEF.md", "EVIDENCE_MANIFEST.md", "mechanical_summary.json", "screening/L3_MATRIX.json", "screening/L3_RESULTS.json", "screening/L3_RESULTS.md", "qa/raw_qa.json", "qa/deck_diff_qa.json", "qa/provenance.json", "qa/execution_summary.json", "qa/transformation_registry.json", "visualization/manifest.json", "qa/visualization_qa.json"}
    with zipfile.ZipFile(zip_path, "r") as archive:
        ...
        if not any(name.startswith("references/") for name in names_in_zip):
            failures.append("L3 references missing")
    if failures:
    ...
    record.update({"status": "PASS", "experiment_id": EXP.name, "package_path": PACKAGE_RELATIVE, "canonical_zip_sha256": sha256(zip_path), "canonical_zip_bytes": zip_path.stat().st_size, "logical_new_point_count": 2, "logical_new_case_count": 4, "actual_new_physical_solve_count": execution["actual_new_physical_solve_count"], "validation_new_physical_solve_count": 0, "reference_l3_1p6_raw_count": 2, "unauthorized_extra_solve_count": 0, "raw_files_modified": 0, "scientific_analysis_performed": False, "zip_committed": False, "drive_mirror_status": "PENDING_AFTER_COMMIT"})
