#!/usr/bin/env python3
"""Minimal visualization QA for the high-range RJ1 trend plots."""
...
COMPARISON_NAMES = ("RJ1_FULL_TREND_0001.html", "RJ1_FULL_TREND_0011.html")
...
def check_entry(entry: dict[str, Any], failures: list[str]) -> None:
    ...
    for index, source_name in enumerate(entry.get("source_raw_paths", [])):
        ...
        if not source.is_file():
            ...
            failures.append(f"source raw hash mismatch: {source_name}")
    if entry.get("stage") == "standalone":
        ...
        if not str(input_path).startswith("/tmp/rj1-highrange-") or input_path.exists():
            failures.append(f"comparison temporary CSV retained: {entry.get('output_path')}")
        if entry.get("temporary_input_deleted") is not True or len(entry.get("comparison_cases", [])) != 8:
...
def main() -> int:
    ...
    if len(standalone) != 24 or len(comparisons) != 2:
    ...
    if {entry.get("output_path") for entry in standalone} != expected_standalone:
        failures.append("standalone paths are not exactly three per new run")
    if {entry.get("output_path") for entry in comparisons} != expected_comparison:
        failures.append("comparison paths are not exactly the authorized full-trend files")
    ...
    qa = {
        "schema": "rj1-highrange-visualization-qa-v1",
