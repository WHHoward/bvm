#!/usr/bin/env python3
"""Build and verify the targeted 4x1 closure evidence ZIP."""
...
PACKAGE_RELATIVE = "handoff/bvm-qb-l1-l3-bj2-targeted-closure-v1-20260914_raw_handoff.zip"
...
def main() -> int:
    clean_cache()
    qa_names = ("raw_qa.json", "deck_diff_qa.json", "provenance.json", "execution_summary.json", "transformation_registry.json", "independent_closure.json", "visualization_qa.json")
    qa = {name: json.loads((EXP / "qa" / name).read_text(encoding="utf-8")) for name in qa_names}
    state = json.loads((EXP / "screening/closure_state.json").read_text(encoding="utf-8"))
    mechanical = json.loads((EXP / "mechanical_summary.json").read_text(encoding="utf-8"))
    failures: list[str] = []
    for name in ("raw_qa.json", "deck_diff_qa.json", "execution_summary.json", "independent_closure.json", "visualization_qa.json"):
    ...
    if failures:
        raise RuntimeError("cannot package closure evidence: " + "; ".join(failures))
    ...
    required = {
        ...
        "screening/CLOSURE_MATRIX.json", "screening/CLOSURE_TABLE.md", "screening/CLOSURE_RESULTS.json", "screening/CLOSURE_RESULTS.md", "qa/raw_qa.json", "qa/deck_diff_qa.json", "qa/provenance.json", "qa/execution_summary.json", "qa/transformation_registry.json", "qa/independent_closure.json", "visualization/manifest.json", "qa/visualization_qa.json", "RAW_ANALYSIS_HANDOFF_MANIFEST.json",
    }
