...
def main() -> int:
    ...
    for entry in comparisons:
        ...
        outputs.add(output)
    temporary = list(Path("/tmp").glob("bvm-l3-highside-*.csv"))
    ...
    record: dict[str, Any] = {"schema": "bvm-qb-l3-highside-visualization-qa-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": "PASS" if not failures else "FAIL", "standalone_html_count": len(standalone), "comparison_html_count": len(comparisons), "phase_page_count": phase_pages, "per_run_standalone_counts": per_run, "temporary_inputs_deleted": not temporary, "derived_csv_present": bool(derived), "raw_direct_standalone": True, "semantic_completeness_gates": {"raw_direct": "PASS" if not failures else "FAIL", "comparison_present": "PASS" if comparisons else "FAIL", "no_unknown_axis": "PASS" if not any("Unknown axis" in item for item in failures) else "FAIL"}, "failures": failures}
