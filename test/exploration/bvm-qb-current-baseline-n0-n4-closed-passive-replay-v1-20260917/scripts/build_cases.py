...
def write_case_files(family: str, mask: str, args: argparse.Namespace, attempt: Path, selected: dict[str, Path], replay_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if directory.exists():
        existing = [directory / name for name in ("raw.csv", "stdout.txt", "stderr.txt", "run.log", "metadata.json") if (directory / name).exists()]
        actual = directory / "actual_deck.cir"
        stimulus = directory / "stimulus.inc"
        if existing or not actual.is_file() or not stimulus.is_file():
            raise RuntimeError(f"cannot resume incomplete case with conflicting artifacts: {directory}")
        return {"run_id": case_id(family, mask), "family": family, "mask": mask, "path": repo_rel(actual), "actual_deck": {"path": repo_rel(actual), "sha256": sha256(actual), "bytes": actual.stat().st_size}, "deck_compatibility": {"path": repo_rel(directory / "deck.cir"), "sha256": sha256(directory / "deck.cir") if (directory / "deck.cir").is_file() else None, "bytes": (directory / "deck.cir").stat().st_size if (directory / "deck.cir").is_file() else None, "identical_to_actual_deck": (directory / "deck.cir").is_file() and sha256(actual) == sha256(directory / "deck.cir")}, "stimulus": {"path": repo_rel(stimulus), "sha256": sha256(stimulus), "bytes": stimulus.stat().st_size, "mode": "generated", "mask": mask}, "probes": {"family": family, "generated_by": repo_rel(Path(__file__)), "exhaustive": True}}
...
def solve_case(case_record: dict[str, Any], args: argparse.Namespace, attempt: Path, provenance: dict[str, Any]) -> None:
    directory = (REPO / case_record["path"]).parent
...
def finalize_attempt(args: argparse.Namespace, attempt: Path, provenance: dict[str, Any]) -> None:
...
def execute_attempt(args: argparse.Namespace, attempt: Path, provenance: dict[str, Any], selected: dict[str, Path]) -> None:
    ...
    for mask in MASKS:
        ...
        solve_case(record, args, attempt, provenance)
    finalize_attempt(args, attempt, provenance)


def run_all(args: argparse.Namespace) -> None:
    attempt, provenance, selected, _source_manifest = prepare_attempt(args)
    execute_attempt(args, attempt, provenance, selected)


def resume_attempt(args: argparse.Namespace) -> None:
    attempt = SERIES / "runs" / args.resume_attempt
    if not attempt.is_dir():
        raise RuntimeError(f"resume attempt does not exist: {attempt}")
    provenance = read_json(SERIES / "provenance.json")
    if provenance.get("run_order"):
        raise RuntimeError(f"resume is only for the pre-solve A001 failure; current run_order={provenance.get('run_order')}")
    if provenance.get("parent", {}).get("head") != git_head() or provenance.get("parent", {}).get("remote_bvm_master") != remote_head():
        raise RuntimeError("resume parent/remote changed")
    source_map: dict[str, Path] = {}
    for item in provenance["source_manifest"]["sources"]:
        role = item["role"]
        if role in {"JJ_MODEL", "BVM", "QB", "JTL"}:
            source_map[role] = REPO / item["snapshot_path"]
    if set(source_map) != {"JJ_MODEL", "BVM", "QB", "JTL"}:
        raise RuntimeError("resume source snapshot is incomplete")
    provenance["tool_repair"] = {"path": repo_rel(Path(__file__)), "sha256": sha256(Path(__file__)), "reason": "fixed cwd from actual_deck.cir to its case directory; no solver process started before repair", "physical_solve_before_repair": False, "raw_mutated": False}
    write_json(SERIES / "provenance.json", provenance)
    execute_attempt(args, attempt, provenance, source_map)
