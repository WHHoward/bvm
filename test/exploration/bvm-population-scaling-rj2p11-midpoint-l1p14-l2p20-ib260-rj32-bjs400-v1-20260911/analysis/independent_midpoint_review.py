...
def raw_path(rj2: int, mask: str) -> Path:
    if rj2 == 11:
        return EXP / "runs" / f"ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P11_{mask}" / "raw.csv"
...
def main() -> int:
    ...
    for rj2 in (11, 12):
        for mask in MASKS:
            ...
            if rj2 == 12 and raw_hash != references["references"][mask]["artifacts"]["raw.csv"]["sha256"]:
            ...
            records[case_id] = {
                "rj2_ohm": rj2,
                "mask": mask,
                "active_bvms_from_mask": [f"BVM{index}" for index, bit in enumerate(mask, 1) if bit == "1"],
    ...
    if execution.get("status") != "PASS" or execution.get("solver_solve_invocations") != 4 or execution.get("exact_new_physical_solve_count") != 4 or execution.get("reference_case_count") != 4 or execution.get("unauthorized_extra_solves") != 0 or execution.get("run_order") != list(NEW_RUNS):
        failures.append("execution exactness guard failed")
    independent_source = (EXP / "analysis/independent_midpoint_review.py").read_text(encoding="utf-8")
    ...
    new_records = {mask: records.get(f"RJ2P11_{mask}", {}) for mask in MASKS}
    primary_outcome = recorded.get("outcome_category_from_recorded_evidence")
    adversarial = {
        "mask_semantics_b3b2b1b0": all(record.get("active_bvms_from_mask") == [f"BVM{index}" for index, bit in enumerate(mask, 1) if bit == "1"] for mask, record in new_records.items()),
        ...
        "0001_single_not_false_two_response": new_records["0001"].get("oracle_count") == 1 and new_records["0001"].get("response_records", [{} , {}])[1]["complete"] is False,
        "0011_second_response_explicitly_checked": len(new_records["0011"].get("response_records", [])) == 4 and "phase_ordered_BJ1_BJ2_JTL1_to_JTL6" in new_records["0011"].get("response_records", [{}, {}])[1].get("checks", {}),
        "0111_fourth_post_read_explicitly_checked": len(new_records["0111"].get("response_records", [])) == 4 and "phase_ordered_BJ1_BJ2_JTL1_to_JTL6" in new_records["0111"].get("response_records", [{}, {}, {}, {}])[3].get("checks", {}),
        "1111_fourth_response_explicitly_checked": len(new_records["1111"].get("response_records", [])) == 4 and "terminal_corresponds_to_JTL6" in new_records["1111"].get("response_records", [{}, {}, {}, {}])[3].get("checks", {}),
        "rj2p12_comparison_references_present": all(f"RJ2P12_{mask}" in records for mask in MASKS),
    ...
    result = {
        "schema": "bjs400-rj2p11-midpoint-independent-review-v1",
        ...
        "new_case_response_candidates": {mask: new_records[mask].get("oracle_count") for mask in MASKS},
        "reference_case_response_candidates": {mask: records.get(f"RJ2P12_{mask}", {}).get("oracle_count") for mask in MASKS},
    ...
    review_lines = [
        ...
        "",
        f"- Cases checked: {len(records)}; new physical solver invocations checked: {execution.get('solver_solve_invocations')}; RJ2=12 comparison cases: {len(MASKS)}.",
    ...
    print(json.dumps({"status": result["status"], "cases_checked": len(records), "recorded_outcome_category": primary_outcome, "new_response_candidates": {mask: new_records[mask].get("oracle_count") for mask in MASKS}, "adversarial_probes": adversarial, "failures": failures}, ensure_ascii=False, indent=2))
