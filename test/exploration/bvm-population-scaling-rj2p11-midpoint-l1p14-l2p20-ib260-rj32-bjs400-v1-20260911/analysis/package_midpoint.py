#!/usr/bin/env python3
"""Build and mechanically verify the RJ2=11 midpoint raw handoff."""
...
PACKAGE_RELATIVE = "handoff/bvm-population-scaling-rj2p11-midpoint-l1p14-l2p20-ib260-rj32-bjs400-v1-20260911_raw_handoff.zip"
sys.path.insert(0, str(REPO / "scripts"))
from build_experiment_package import build_package  # noqa: E402


NEW_RUNS = (
    "ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P11_0001",
    "ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P11_0011",
    "ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P11_0111",
    "ARRAY_L1P14_L2P20_IB260_RJ32_RJ2P11_1111",
)
REFERENCE_RUNS = (
...
def main() -> int:
    ...
    if raw.get("status") != "PASS" or raw.get("artifact_validity") != "VALID" or deck.get("status") != "PASS" or response.get("status") != "PASS" or independent.get("status") != "PASS" or viz.get("status") != "PASS" or mechanical.get("status") != "PASS":
        raise RuntimeError("one or more RJ2=11 midpoint evidence QA artifacts are not PASS")
    if execution.get("status") != "PASS" or execution.get("exact_new_physical_solve_count") != 4 or execution.get("solver_solve_invocations") != 4 or execution.get("reference_case_count") != 4 or execution.get("unauthorized_extra_solves") != 0 or tuple(execution.get("run_order", ())) != NEW_RUNS:
        raise RuntimeError("execution summary is not exact 4-new/4-reference PASS")
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        for run_id in REFERENCE_RUNS:
            for suffix in ("deck.cir", "raw.csv", "metadata.json", "run.log"):
                if f"references/rj2p12/{run_id}/{suffix}" not in name_set:
                    failures.append(f"missing RJ2=12 reference artifact: {run_id}/{suffix}")
    ...
    for run_id in REFERENCE_RUNS:
        for suffix in ("raw.csv", "deck.cir", "metadata.json", "run.log"):
            local = EXP / "references/rj2p12" / run_id / suffix
            with zipfile.ZipFile(zip_path) as archive:
                name = f"references/rj2p12/{run_id}/{suffix}"
    ...
    qa.update({"status": "PASS", "package_path": PACKAGE_RELATIVE, "new_physical_solve_count": 4, "reference_case_count": 4, "logical_case_count": 8, "unauthorized_extra_solves": 0, "scientific_analysis_performed": False, "canonical_timestep_ps": 0.1, "standalone_html_count": 24, "comparison_html_count": 4, "control_qa_status": control.get("status"), "canonical_zip_sha256": sha256(zip_path), "canonical_zip_bytes": zip_path.stat().st_size, "zip_committed": False, "package_authority": "Git authoritative; Drive analysis mirror only"})
    qa_path.write_text(json.dumps(qa, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({"status": "PASS", "package": str(zip_path), "sha256": qa["canonical_zip_sha256"], "bytes": qa["canonical_zip_bytes"], "new_physical_solve_count": 4, "reference_case_count": 4, "scientific_analysis_performed": False}, ensure_ascii=False, indent=2))
    return 0
