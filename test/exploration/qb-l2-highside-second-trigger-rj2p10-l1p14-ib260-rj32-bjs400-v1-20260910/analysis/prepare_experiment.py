#!/usr/bin/env python3
"""Register the fixed-point high-L2 second-trigger experiment.

This script performs no physical solve. It creates the six registered L2
decks and verifies the exact L2=2.0pH/RJ2=10ohm historical baseline before execution.
"""
...
AUTH = REPO / "test/exploration/qb-rj2-highside-rearm-boundary-l1p14-l2p20-ib260-rj32-bjs400-v1-20260910"
SOLVER = REPO / "build/josim-cli"
SOURCE_TEMPLATE = EXP / "inputs/array_fixture_template.cir"
BQ400 = EXP / "inputs/BQ_parameterized_bjs400.cir"
BQ_RJ2 = EXP / "inputs/BQ_parameterized_bjs400_rj2.cir"
BQ_L2_RJ2 = EXP / "inputs/BQ_parameterized_bjs400_l2_rj2.cir"
...
AUTH_SOURCE_MANIFEST_SHA256 = "31752f9e74b95d1d787d88c146a1a7d650da5d37ac3a4a3a035494a604ef1b4d"
AUTH_PACKAGE_SHA256 = "9b7422ff45a55f002c4b4f9d0501e63f3b4d48d7a1e19f42641c122b8f1fc42a"
AUTH_GIT_HEAD = "345d54e6ebf9f0e8b39af616d942ff800ea4e0a7"
...
L2_VALUES = (2.4, 2.8, 3.2)
MASKS = ("0001", "0011")
CONTROL_KINDS = ("WL", "BL", "SE")
NEW_RUNS = tuple(
    f"ARRAY_L1P14_L2P{int(l2 * 10):02d}_IB260_RJ32_RJ2P10_{mask}"
    for l2 in L2_VALUES
    for mask in MASKS
)
...
RUN_TO_L2 = {
    run_id: l2
    for l2 in L2_VALUES
    for run_id in (f"ARRAY_L1P14_L2P{int(l2 * 10):02d}_IB260_RJ32_RJ2P10_0001", f"ARRAY_L1P14_L2P{int(l2 * 10):02d}_IB260_RJ32_RJ2P10_0011")
}
RUN_TO_RJ2 = {run_id: 10.0 for run_id in NEW_RUNS}
...
def build_deck(l2: float, mask: str) -> str:
    ...
    new_bq_include = ".include ../../inputs/BQ_parameterized_bjs400_l2_rj2.cir"
    ...
    params = (
        ...
        f".param RJ2_VALUE={FIXED['RJ2_ohm']:g}\n"
    )
...
def normalized_deck(text: str) -> tuple[str, ...]:
    ...
    for line in text.splitlines():
        if line.startswith(".param L1_VALUE=") or line.startswith(".param IB_VALUE=") or line.startswith(".param RJ1_VALUE=") or line.startswith(".param L2_VALUE=") or line.startswith(".param RJ2_VALUE="):
            continue
        line = line.replace("../../inputs/BQ_parameterized_bjs400_l2_rj2.cir", "../../inputs/BQ_parameterized_bjs400_rj2.cir")
...
def write_reuse_manifest() -> dict[str, Any]:
    ...
    if not AUTH.is_dir() or sha256(AUTH / "SOURCE_MANIFEST.json") != AUTH_SOURCE_MANIFEST_SHA256:
        raise RuntimeError("RJ2 damping source manifest is missing or changed")
    authority_package = AUTH / "handoff/qb-rj2-highside-rearm-boundary-l1p14-l2p20-ib260-rj32-bjs400-v1-20260910_raw_handoff.zip"
    if not authority_package.is_file() or sha256(authority_package) != AUTH_PACKAGE_SHA256:
        raise RuntimeError("RJ2 damping source package is missing or changed")
    ...
    for run_id, spec in SOURCE_REUSE.items():
        ...
        if params.get("L1_pH") != 1.4 or params.get("L2_pH") != 2.0 or params.get("IBias_uA") != 260.0 or params.get("RJ1_ohm") != 32.0 or params.get("RJ2_ohm") != 10.0:
            failures.append("fixed parameter mismatch")
        if metadata.get("rj2_ohm") != 10.0 or metadata.get("l2_pH", params.get("L2_pH")) != 2.0:
            failures.append("RJ2 mismatch")
        ...
        if normalized_deck(source_deck) != normalized_deck(expected_deck):
            failures.append("deck/include/topology/RJ2-parameterization mismatch")
        ...
        if failures:
            raise RuntimeError(f"L2=2.0pH/RJ2=10ohm exact reuse verification failed for {run_id}: {'; '.join(failures)}")
        references[run_id] = {
            ...
            "parameters": {"L1_pH": 1.4, "L2_pH": 2.0, "IBias_uA": 260.0, "RJ1_ohm": 32.0, "RJ2_ohm": 10.0},
            "topology": "ARRAY",
            "mask": spec["mask"],
            "physical_solve_this_experiment": False,
            "reuse_reason": "exact L2=2.0pH/RJ2=10ohm baseline under fixed L1/RJ1/IBias, BJS400 fixture, stored history, mask semantics, solver/timestep/stop-time and source/archive hashes match",
            "comparability": {
                "L1_1p4_L2_2p0_IBias_260_RJ1_32": "PASS",
                "L2_2p0_RJ2_10ohm": "PASS",
    ...
    record = {
        "schema": "bjs400-rj2-highside-reuse-manifest-v1",
...
def write_source_manifest() -> dict[str, Any]:
    ...
    for name, role in (
        ...
        ("BQ_parameterized_bjs400_rj2.cir", "audited BJS400 QB include with RJ2 parameter exposure"),
        ("BQ_parameterized_bjs400_l2_rj2.cir", "derived BJS400 QB include with L2 parameter exposure at fixed RJ2"),
    ):
    ...
    record = {
        "schema": "bjs400-l2-highside-second-trigger-source-manifest-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        ...
        "fixed_working_point": {"L1_pH": 1.4, "L2_pH": 2.0, "IBias_uA": 260.0, "RJ1_ohm": 32.0, "BJS_area": 4, "BJS_Ic_uA": 400},
        "l2_values_new_pH": list(L2_VALUES),
        "l2_baseline_reused_pH": 2.0,
        "fixed_rj2_ohm": 10.0,
        "l2_parameterization_delta": {"authority_line": "L2 3 4 2p", "derived_line": "L2 3 4 {L2_VALUE}", "derived_source_sha256": sha256(BQ_L2_RJ2)},
...
def generate_decks() -> dict[str, Any]:
    ...
    for run_id in NEW_RUNS:
        ...
        records[run_id] = {"L2_pH": RUN_TO_L2[run_id], "RJ2_ohm": FIXED["RJ2_ohm"], "mask": RUN_TO_MASK[run_id], "parameters": {**FIXED, "L2_pH": RUN_TO_L2[run_id]}, "path": str(deck.relative_to(REPO)), "sha256": sha256(deck), "bytes": deck.stat().st_size}
    return records


def bq_delta() -> dict[str, Any]:
    base = BQ_RJ2.read_text(encoding="utf-8").splitlines()
    variant = BQ_L2_RJ2.read_text(encoding="utf-8").splitlines()
    changes = [(index + 1, before, after) for index, (before, after) in enumerate(zip(base, variant)) if before != after]
    valid = len(base) == len(variant) and len(changes) == 2 and any("L2 3 4 2p" in before and after == "L2 3 4 {L2_VALUE}" for _line, before, after in changes) and any("Only the active BQ element values for L1, RJ1, RJ2 and IBias" in before and "L2" in after for _line, before, after in changes)
...
def verify_decks() -> dict[str, Any]:
    ...
    records: dict[str, Any] = {}
    bq = BQ_L2_RJ2.read_text(encoding="utf-8")
    base_bq = BQ_RJ2.read_text(encoding="utf-8")
    if "BJs 1 2 jjmit area=4" not in bq or "BJs 1 2 jjmit area=3" in bq:
        failures.append("BJS400 source is not exactly area=4")
    if "L2 3 4 {L2_VALUE}" not in bq or "L2 3 4 2p" in bq or "RJ2 4 0 {RJ2_VALUE}" not in bq:
        failures.append("L2-parameterized BQ source is not active")
    if "L2 3 4 2p" not in base_bq or "RJ2 4 0 {RJ2_VALUE}" not in base_bq:
        failures.append("audited RJ2 authority include is missing")
    for run_id in NEW_RUNS:
        deck = EXP / "runs" / run_id / "deck.cir"
        text = deck.read_text(encoding="utf-8")
        l2 = RUN_TO_L2[run_id]
        mask = RUN_TO_MASK[run_id]
        for required in (".param L1_VALUE=1.4p", ".param IB_VALUE=260u", ".param RJ1_VALUE=32", f".param L2_VALUE={l2:g}p", ".param RJ2_VALUE=10", ".include ../../inputs/BQ_parameterized_bjs400_l2_rj2.cir", ".tran 0.1p 200p", "P(BJS|XBQ1) V(BJS|XBQ1) I(BJS|XBQ1)"):
        ...
        records[run_id] = {"L2_pH": l2, "RJ2_ohm": FIXED["RJ2_ohm"], "mask": mask, "deck_sha256": sha256(deck), "deck_bytes": deck.stat().st_size}
    if len(normalized) != 1:
    ...
    return {
        "schema": "bjs400-l2-highside-second-trigger-deck-diff-preflight-v1",
...
def preflight_markdown(record: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "",
        f"- L2=2.0pH/RJ2=10ohm reuse comparability: `{record['reuse_comparability']['status']}`.",
...
def main() -> int:
    ...
    record = {
        "schema": "bjs400-l2-highside-second-trigger-preflight-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "status": "PASS",
        "head": head(),
