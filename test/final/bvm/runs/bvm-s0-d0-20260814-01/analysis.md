# Run analysis: `bvm-s0-d0-20260814-01`
...
- PASS observation: all three D0 guards hold in both state windows
  (>=2 finite samples; per-component p2p <= 0.02 rad; L-inf separation
  >= 0.10 rad between init means in both state windows).
...
- INVALID conditions: missing column, NaN/Inf, non-monotonic time, truncated
  window, nonzero solver exit, pre-existing run root, unregistered change.

## Inputs and provenance

- Manifest: `manifest.yaml`
- Netlist/include snapshot: `inputs/` (copied `jjmit.cir`, `bvm_cell.cir`
  closure; three instrumented netlists `init_positive.cir`,
  `init_negative.cir`, `no_init_control.cir`)
- JoSIM binary/version/SHA-256: `/usr/local/bin/josim-cli` v2.7.02a34ee
  `d6913de92f71ee57bcb519f40c9fb60f3b51f404ec46ef8b00d3a7f3dfdf4b9c`
...
- Exit/log status: all three `josim-cli` runs exit 0, empty stderr, stdout
  preserved; no solver warnings observed.
- CSV/time/column checks: 798 samples per run; time strictly increasing to
  ...
  (time, P/V JM1, P/V JM2, V(SL1), I(L_SL|XBVM1)).
...
### init_activity `[9,31) ps` phase–area (AC3)
...
### Source port (12-ohm load) — descriptive (AC3)
...
- Whether either initialized JM1/JM2 signature is a published logical state
  ...
  timestep ladder (design document "What an accepted D0 enables").
...
Reason: artifacts are valid (QA passed, direct-JJ probes present, closure
immutable) and the init procedures produce a large, stable JM1 separation
(11.82 rad, guard 3 passed in both windows), but guard 2 fails: JM2 p2p in
state_early is 0.0708 rad > 0.02 rad in both init cases. Per the design
document, an unmet guard keeps D0 a valid exploratory artifact with an
INCONCLUSIVE conclusion; D0 does not advance S0.
