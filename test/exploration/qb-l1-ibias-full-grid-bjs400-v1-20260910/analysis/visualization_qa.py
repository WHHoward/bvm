#!/usr/bin/env python3
"""Minimal visualization QA for the complete L1 x IBias grid."""
...
COMPARISON_NAMES = ("L1_IBIAS_GRID_0001_COMPARE.html", "L1_IBIAS_GRID_0011_COMPARE.html")
...
def check_entry(entry: dict[str, Any], failures: list[str]) -> None:
    ...
    if entry.get("stage") == "standalone":
        ...
        if not str(input_path).startswith("/tmp/bjs400-full-grid-") or input_path.exists():
        ...
        if len(entry.get("comparison_cases", [])) != 12 or entry.get("temporary_input_sha256") is None:
...
def main() -> int:
    ...
    if len(standalone) != 30 or len(comparison) != 2:
    ...
    qa = {
        "schema": "bjs400-l1-ibias-full-grid-visualization-qa-v1",
