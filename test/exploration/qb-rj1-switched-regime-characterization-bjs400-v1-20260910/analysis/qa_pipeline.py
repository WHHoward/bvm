#!/usr/bin/env python3
"""Minimal raw/deck QA and registered mechanical summary for RJ1."""
...
from prepare_experiment import BQ400, CONTROL_KINDS, NEW_RUNS, REUSE_RUNS, RUN_TO_MASK, RUN_TO_RJ1, control_line  # noqa: E402
...
RAW_LABELS = [
    "V(COMMON_SL)", "P(B_JSL8)", "V(B_JSL8)", "I(B_JSL8)",
    "V(QBIN)", "V(QBOUT)", "I(LIN|XBQ1)", "V(LIN|XBQ1)",
    "P(BJ1|XBQ1)", "V(BJ1|XBQ1)", "I(BJ1|XBQ1)",
    "I(RJ1|XBQ1)", "V(RJ1|XBQ1)", "I(L1|XBQ1)", "V(L1|XBQ1)",
    "I(L2|XBQ1)", "V(L2|XBQ1)", "P(BJ2|XBQ1)", "V(BJ2|XBQ1)",
    "I(BJ2|XBQ1)", "I(BJSL8)",
]
WINDOWS = (
...
def window_values(time_s: tuple[float, ...], values: tuple[float, ...], start: float, end: float) -> tuple[tuple[float, ...], tuple[float, ...]]:
    indexes = window_indices(time_s, start, end)
    return tuple(time_s[index] for index in indexes), tuple(values[index] for index in indexes)


def phase_diagnostic(trace: Any, label: str) -> dict[str, Any]:
    phase = continuous_unwrap(tuple(trace.column(label)))
    pre = window_indices(trace.time, 101e-12, 110e-12)
    post = window_indices(trace.time, 110e-12, 200e-12)
    if not pre or not post:
        return {"status": "UNKNOWN", "reason": "registered reference/post window has no stored samples", "not_event_count": True}
    baseline = phase[pre[0]]
    relative = tuple((phase[index] - baseline) / (2.0 * math.pi) for index in post)
    half_turn = next((index for index, value in zip(post, relative) if value >= 0.5), None)
    return {
        "status": "DERIVED",
        "label": label,
        "baseline_window_ps": [101.0, 110.0],
        "baseline_reference_time_ps": trace.time[pre[0]] * 1e12,
        "raw_unit": "rad",
        "display_unit": "turns",
        "conversion": "continuous_unwrap(raw_rad)/(2*pi)",
        "first_half_turn_timing_diagnostic_ps": trace.time[half_turn] * 1e12 if half_turn is not None else None,
        "first_half_turn_diagnostic_found": half_turn is not None,
...
def mechanical_case(case_id: str, trace: Any) -> dict[str, Any]:
    def values(label: str) -> tuple[float, ...]:
        return tuple(float(value) for value in trace.column(label))

    result: dict[str, Any] = {
        "case_id": case_id,
        "rj1_ohm": 12.0 if case_id in REUSE_RUNS else RUN_TO_RJ1[case_id],
        ...
        "notes": [
            ...
            "First +0.5-turn value is a timing diagnostic, not an event time.",
            "Phase displacement is not an SFQ count.",
        ],
    }
    for label in ("I(L1|XBQ1)", "I(L2|XBQ1)", "I(RJ1|XBQ1)", "V(RJ1|XBQ1)", "I(B_JSL8)"):
        ...
        result["current_voltage_descriptives"][label] = {
            "max": max(column),
            "min": min(column),
            "max_abs": max(abs(value) for value in column),
        }
    for label in ("I(B_JSL8)", "I(LIN|XBQ1)"):
        column = values(label)
        for name, start, end in WINDOWS:
            times, selected = window_values(trace.time, column, start, end)
...
def deck_diff() -> dict[str, Any]:
    ...
    same_mask_rj: dict[str, set[tuple[str, ...]]] = {"0001": set(), "0011": set()}
    ...
    for run_id in NEW_RUNS:
        ...
        if any(text.count(f"XBVM{index} ") != 1 for index in range(1, 5)):
            failures.append(f"{run_id}: BVM topology mismatch")
        for instance in range(1, 5):
        ...
        same_mask_rj[mask].add(tuple(line for line in text.splitlines() if line.startswith(".param RJ1_VALUE=")))
    ...
    if any(len(values) != 3 for values in same_mask_rj.values()):
        failures.append("same-mask RJ1 deck set is incomplete")
    return {
        "schema": "rj1-switched-regime-deck-diff-qa-v1",
        "status": "PASS" if not failures else "FAIL",
        "new_run_count": len(NEW_RUNS),
        "reused_case_count": len(REUSE_RUNS),
        "fixed_working_point": {"L1_pH": 1.6, "IBias_uA": 260.0, "BJS_area": 4, "RJ1_baseline_ohm": 12.0},
        "same_mask_only_rj1_difference": len(same_mask_rj["0001"]) == 3 and len(same_mask_rj["0011"]) == 3,
...
def main() -> int:
    ...
    for case_id in ALL_CASES:
        ...
        if case_id in REUSE_RUNS and pre_hashes[case_id] != reuse_manifest["references"][case_id]["artifacts"]["raw.csv"]["source_sha256"]:
            raw_failures.append(f"{case_id}: reuse raw hash mismatch")
    ...
    raw_qa = {
        "schema": "rj1-switched-regime-raw-qa-v1",
    ...
    decks = deck_diff()
    provenance = {
        "schema": "rj1-switched-regime-provenance-v1",
        "experiment_id": EXP.name,
    ...
    transformations = {
        "schema": "rj1-switched-regime-transformation-registry-v1",
        "raw_immutable": True,
        "scientific_analysis_performed": False,
        "transformations": [
            {"name": "mechanical_summary", "operation": "actual-grid descriptive extrema, phase diagnostic and trapezoid areas", "scope": "registered mechanical arithmetic only"},
    ...
    write_json(EXP / "mechanical_summary.json", {
        "schema": "rj1-switched-regime-mechanical-summary-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
