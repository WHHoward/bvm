...
REFERENCE_CASES = {
    "R0_CANONICAL_L1_1p4_L3_1p3_BJ2_2p0": {"0011": EXP / "references/canonical_l3p13/0011/raw.csv", "0111": EXP / "references/canonical_l3p13/0111/raw.csv"},
    "R1_L1_RESCUE_ONLY": {"0011": EXP / "references/l1p12_l3p13_bj2p20/0011/raw.csv", "0111": EXP / "references/l1p12_l3p13_bj2p20/0111/raw.csv"},
    "R2_L3_BJ2_BARRIER": {"0011": EXP / "references/l3p20_bj2p22/0011/raw.csv", "0111": EXP / "references/l3p20_bj2p22/0111/raw.csv"},
}
VIEW_LABELS = {
...
def focused_comparison(cases: list[tuple[str, Path]], branch: str) -> dict[str, Any]:
    ...
    try:
        ...
        command = plot(temporary, output, conditioned_labels, f"{branch} — canonical / L1-only / L3+BJ2 / target — 108–150 ps")
...
def main() -> int:
    ...
    for mask, raw in sorted(raw_by_mask.items()):
        run_id = f"TARGET_L1_1p2_L3_2_BJ2_2p2_{mask}"
    ...
    for mask, branch in (("0011", "N2"), ("0111", "N3")):
        ...
        comparison_cases.append(("TARGET_L1_1p2_L3_2_BJ2_2p2", raw_by_mask[mask]))
