#!/usr/bin/env python3
"""Execute the two registered L1/L3/BJ2 closure screening cases."""
...
TABLE_JSON = EXP / "screening/CLOSURE_RESULTS.json"
TABLE_MD = EXP / "screening/CLOSURE_RESULTS.md"
...
OUTCOME_LABELS = {
    "S": "DIRECT_2_TO_3_TRIPLE_PARAMETER_CANDIDATE",
    "A": "N3_SUPPRESSED_N2_REARM_INSUFFICIENT",
    "B": "N2_RESCUED_N3_FOURTH_REOPENED",
    "C": "TRIPLE_PARAMETER_DECOUPLING_FAILED",
...
def head_relation(sealed: str) -> dict[str, Any]:
    ...
    allowed = {
        ...
        f"{prefix}/screening/CLOSURE_MATRIX.json",
        f"{prefix}/screening/CLOSURE_TABLE.md",
    }
...
def state_template(preflight: dict[str, Any], relation: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "bvm-qb-l1-l3-bj2-targeted-closure-state-v1",
        ...
        "maximum_total_solve_count": 2,
        "screening_cases": {},
        "run_order": [],
        "candidate_class": None,
        "outcome": None,
        "validation": {"status": "RESERVED_REQUIRES_SCIENTIFIC_REVIEW", "performed": False},
    }
...
def run_case(point: dict[str, Any], mask: str, *, run_kind: str) -> dict[str, Any]:
    ...
    metadata = {
        "schema": "bvm-qb-l1-l3-bj2-targeted-closure-run-metadata-v1",
        "experiment_id": EXP.name,
        "run_id": run_id,
        "run_kind": run_kind,
...
def analyse(point: dict[str, Any], cases: dict[str, Any], *, force: bool = False) -> dict[str, Any]:
    if force or not RESULT_PATH.is_file():
        command = [sys.executable, str(EXP / "analysis/closure_analysis.py"), "--point-id", point["point_id"], "--changes", json.dumps(point["changes"], separators=(",", ":")), "--n2", str(REPO / cases["0011"]["raw_path"]), "--n3", str(REPO / cases["0111"]["raw_path"])]
        completed = subprocess.run(command, cwd=REPO, check=False)
        if completed.returncode != 0 or not RESULT_PATH.is_file():
            raise RuntimeError(f"closure raw analysis failed: {point['point_id']}")
    ...
    if actual != expected:
        raise RuntimeError(f"closure result/raw hash binding failed: {point['point_id']}")
    return result
...
def write_table(state: dict[str, Any], result: dict[str, Any] | None) -> None:
    ...
    row.update({"L1_pH": 1.2, "L3_pH": 2.0, "BJ2_area": 2.2})
    write_json(TABLE_JSON, {"schema": "bvm-qb-l1-l3-bj2-targeted-closure-results-v1", "experiment_id": EXP.name, "updated_at_local": now(), "row": row, "references": {"R0": "references/canonical_l3p13", "R1": "references/l1p12_l3p13_bj2p20", "R2": "references/l3p20_bj2p22"}, "phase_navigation_only": True, "not_sfq_count": True})
    lines = ["# L1/L3/BJ2 targeted closure results", "", "| point | L1 pH | L3 pH | BJ2 area | control N2 | control N3 | N2 candidates | N3 candidates | N2 class | N3 class | result class | validation |", "|---|---:|---:|---:|---|---|---:|---:|---|---|---|---|"]
    values = [row["point_id"], 1.2, 2.0, 2.2, row["control_n2"] or "", row["control_n3"] or "", row["N2_candidate_count"] if row["N2_candidate_count"] is not None else "", row["N3_candidate_count"] if row["N3_candidate_count"] is not None else "", row["N2_classification"] or "", row["N3_classification"] or "", row["candidate_label"] or "", row["validation_status"]]
    lines.append("| " + " | ".join(str(value) for value in values) + " |")
    lines.extend(["", "Three reference pairs are immutable raw context: R0 canonical, R1 L1-only rescue, and R2 previous L3/BJ2 barrier point.", "", "P(...) is raw radians; any turns are phase navigation only and are not SFQ counts.", ""])
...
def write_execution(state: dict[str, Any]) -> None:
    ...
    validation_count = 0
    write_json(SUMMARY, {
        "schema": "bvm-qb-l1-l3-bj2-targeted-closure-execution-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "status": "PASS",
        ...
        "maximum_total_solve_count": 2,
        "conditional_validation_status": "RESERVED_REQUIRES_SCIENTIFIC_REVIEW",
        "unauthorized_extra_solve_count": 0,
        "run_order": run_order,
        "outcome": state.get("outcome"),
        "conditional_validation_performed": False,
...
def main() -> int:
    ...
    if result.get("candidate_class") == "S":
        return finalize(state, result, "DIRECT_2_TO_3_TRIPLE_PARAMETER_CANDIDATE", "clean 2/3 screening pair found; validation requires later scientific review and is not authorized here")
