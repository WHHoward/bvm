#!/usr/bin/env python3
"""Execute exactly the ten newly authorized full-grid runs once."""
...
from prepare_experiment import NEW_RUNS, REUSE_RUNS, NEW_RUN_PARAMETERS, RUN_TO_MASK  # noqa: E402
...
def make_record(run_id: str, head: str, command: list[str], started: str, finished: str, exit_code: int, deck: Path, raw: Path, log: Path) -> dict[str, Any]:
    parameters = NEW_RUN_PARAMETERS[run_id]
    return {
        "schema": "bjs400-l1-ibias-full-grid-run-metadata-v1",
        "experiment_id": EXP.name,
        "run_id": run_id,
        "grid_point": f"L1={parameters['L1_pH']}pH, IBias={parameters['IBias_uA']}uA",
...
def main() -> int:
    ...
    if preflight.get("status") != "PASS" or preflight.get("new_physical_solve_count") != 10 or preflight.get("reused_physical_case_count") != 14:
        raise RuntimeError("preflight is not PASS for exact 10-new/14-reuse matrix")
    relation = head_relation(str(preflight["head"]))
    if relation["status"] != "PASS":
        raise RuntimeError(f"HEAD/preflight relation is not an allowed seal: {relation}")
    ...
    for run_id in NEW_RUNS:
        ...
        if raw.exists() or log.exists() or metadata.exists():
            raise RuntimeError(f"refusing to overwrite existing new-run artifact: {run_dir}")
    ...
    summary = {
        "schema": "bjs400-l1-ibias-full-grid-execution-v1",
        "experiment_id": EXP.name,
        ...
        "exact_new_physical_solve_count": len(NEW_RUNS),
    ...
    print(json.dumps({
        ...
        "new_physical_solve_count": len(NEW_RUNS) - len(failures),
        "reused_physical_case_count": len(REUSE_RUNS),
        "failed_runs": failures,
        "scientific_analysis_performed": False,
