...
def crop_raw(raw: Path, start: float, end: float) -> Path:
    with raw.open(newline="", encoding="utf-8-sig") as stream:
        rows = list(csv.reader(stream))
    if not rows:
        raise RuntimeError(f"empty raw CSV: {raw}")
    headers = rows[0]
    time_index = headers.index("time")
    selected = [row for row in rows[1:] if start <= float(row[time_index]) * 1.0e12 < end]
    handle = tempfile.NamedTemporaryFile(prefix="bvm_baseline_crop_", suffix=".csv", delete=False, dir="/tmp")
...
def render_group(raw: Path, output: Path, signals: list[str], title: str, asset: Path, window: tuple[float, float] | None = None, static: bool = False) -> dict[str, Any]:
    if not signals:
    ...
    if window is not None:
        crop = crop_raw(raw, *window)
        input_path = crop
    with tempfile.NamedTemporaryFile(prefix="bvm_baseline_plot_", suffix=".html", delete=False, dir="/tmp") as handle:
        temporary = Path(handle.name)
    command = [sys.executable, str(plotter_path()), str(input_path), "-x", str(temporary), "-t", "sep_comb", "-c", "dark", "-j", "2pi", "-w", title, "-s", *signals]
    static_path = output.with_suffix(".svg") if static else None
    ...
    return {"output": repo_rel(output), "sha256": sha256(output), "raw_path": repo_rel(raw), "raw_sha256": sha256(raw), "signals": signals, "renderer": repo_rel(plotter_path()), "renderer_arguments": command, "time_unit": "ps", "window_ps": list(window) if window is not None else [0.0, 200.0], "phase_display": "-j 2pi navigation; raw P(...) remains radians", "static_output": repo_rel(static_path) if static_path and static_path.is_file() else None, "static_sha256": sha256(static_path) if static_path and static_path.is_file() else None, "descriptive_only": True}
...
def list_attempt_signals(attempt: Path) -> None:
    for case_dir in sorted(path for path in (attempt / "cases").iterdir() if path.is_dir()):
        case = load_case(case_dir / "raw.csv")
        print(f"RUN: {case_dir.name}")
        print(f"RAW: {repo_rel(case['path'])}")
        print(f"RAW_SHA256: {case['raw_sha256']}")
        for kind in ("voltage", "current", "phase", "other"):
            signals = [name for name in case["headers"] if name != "time" and signal_class(name)[2] == kind]
            print(f"  {kind}: {len(signals)}")
            for signal in signals:
                print(f"    {signal}")
        print("  phase_semantics: raw radians; rad/(2*pi) turns navigation only")


def main() -> int:
    parser = argparse.ArgumentParser(description="Render standalone exhaustive baseline visualizations")
    parser.add_argument("attempt_positional", nargs="?")
    ...
    attempt_name = args.attempt or args.attempt_positional
    if not attempt_name:
        raise RuntimeError("plotting requires ATTEMPT such as A001")
    attempt = SERIES / "runs" / attempt_name
    if not attempt.is_dir():
        raise RuntimeError(f"attempt does not exist: {attempt}")
    provenance = read_json(SERIES / "provenance.json")
    run_ids = list(provenance.get("run_order", []))
    if args.list_signals:
        list_attempt_signals(attempt)
        return 0
    if len(run_ids) != 15:
    ...
    selected_groups = [args.group] if args.group else ([] if args.signals else list(GROUPS))
    group_jobs: list[tuple[Path, Path, list[str], str, tuple[float, float] | None, bool]] = []
    ...
    for run_id, case in cases.items():
        ...
        for group in selected_groups:
            ...
            for quantity, signals in quantities.items():
                output = groups_dir / f"{group}_{quantity}.html" if args.window is None else visualization / run_id / "adhoc" / f"{group}_{quantity}_{args.window[0]:g}_{args.window[1]:g}ps.html"
                group_jobs.append((case["path"], output, signals, f"{run_id} — {group} — {quantity}", tuple(args.window) if args.window else None, args.static))
    workers = min(8, max(1, os.cpu_count() or 1))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(render_group, raw, output, signals, title, asset, window, static) for raw, output, signals, title, window, static in group_jobs]
        for future in futures:
            grouped_entries.append(future.result())
    if args.signals:
        if not args.adhoc:
            raise RuntimeError("--signals requires --adhoc internally")
        adhoc_entries: list[dict[str, Any]] = []
        for run_id, case in cases.items():
            output = visualization / run_id / "adhoc" / ("signals_" + hashlib.sha256("\0".join(args.signals).encode()).hexdigest()[:12] + ".html")
            adhoc_entries.append(render_group(case["path"], output, args.signals, f"{run_id} — adhoc signals", asset, tuple(args.window) if args.window else None, args.static))
        grouped_entries.extend(adhoc_entries)
    if args.group or args.signals:
        write_json(visualization / "plot_manifest.json", {"schema": "bvm-qb-current-baseline-visualization-manifest-v1", "experiment_id": SERIES.name, "attempt": attempt_name, "generated_at": __import__("datetime").datetime.now().astimezone().isoformat(timespec="seconds"), "renderer": repo_rel(plotter_path()), "renderer_defaults": {"layout": "sep_comb", "color": "dark", "phase_option": "2pi", "time_unit": "ps"}, "no_cross_run_comparison": True, "grouped_entries": grouped_entries, "standalone_count": len(grouped_entries), "descriptive_only": True})
        print(json.dumps({"status": "PASS", "attempt": attempt_name, "grouped_pages": len(grouped_entries), "no_cross_run_comparison": True}, ensure_ascii=False, indent=2))
        return 0
    ...
    for run_id in run_ids:
        ...
        group_links = [f"<li><a href=\"{Path(item['output']).name}\">{Path(item['output']).stem}</a></li>" for item in run_group_entries]
        ...
        index_lines.extend([f"## {run_id}", "", f"- [grouped views]({run_id}/groups/index.html)", f"- [all-signal atlas]({run_id}/atlas/index.html)", ""])
    ...
    if plot_qa["status"] != "PASS":
        raise RuntimeError(f"visualization QA failed: {invalid_runtime_pages}")
    print(json.dumps({"status": "PASS", "attempt": attempt_name, "grouped_pages": len(grouped_entries), "atlas_pages": len(atlas_entries), "signals_per_run": {run_id: len([name for name in case["headers"] if name != "time"]) for run_id, case in cases.items()}}, ensure_ascii=False, indent=2))
    return 0
