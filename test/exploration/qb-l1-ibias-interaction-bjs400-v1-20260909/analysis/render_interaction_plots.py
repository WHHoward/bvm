#!/usr/bin/env python3
"""Render compact whole-run plots for the two-by-two interaction evidence."""
...
RUN_PATHS = {
    run_id: EXP / "runs" / run_id for run_id in NEW_RUNS
}
RUN_PATHS.update({
    run_id: EXP / "references/reused" / run_id for run_id in REUSE_RUNS
})
CORNERS = {
    "A": ("ARRAY_L1P20_IB250_0001", "ARRAY_L1P20_IB250_0011"),
    "B": ("ARRAY_IB260_0001", "ARRAY_IB260_0011"),
    "C": ("ARRAY_L1P16_IB250_0001", "ARRAY_L1P16_IB250_0011"),
    "D": ("ARRAY_L1P16_IB260_0001", "ARRAY_L1P16_IB260_0011"),
}
...
def render_comparison(entries: list[dict[str, object]], name: str, case_ids: list[str]) -> None:
    ...
    temp_path = Path(tempfile.mkstemp(prefix="bjs400-interaction-", suffix=".csv", dir="/tmp")[1])
    try:
...
def write_run_summaries() -> list[Path]:
    ...
    for run_id in NEW_RUNS:
        ...
        lines = [
            f"# {run_id} — compact interaction evidence",
            "",
...
def main() -> int:
    ...
    for mask in ("0001", "0011"):
        index = 0 if mask == "0001" else 1
        render_comparison(entries, f"{mask}_INTERACTION_COMPARE", [CORNERS[key][index] for key in ("A", "B", "C", "D")])
    ...
    manifest = {
        "schema": "bjs400-l1-ibias-interaction-flat-visualization-manifest-v1",
        ...
        "corners": {
            "A": {"L1_pH": 2.0, "IBias_uA": 250.0, "case_ids": list(CORNERS["A"])},
            "B": {"L1_pH": 2.0, "IBias_uA": 260.0, "case_ids": list(CORNERS["B"]), "historical_reuse": True},
            "C": {"L1_pH": 1.6, "IBias_uA": 250.0, "case_ids": list(CORNERS["C"])},
            "D": {"L1_pH": 1.6, "IBias_uA": 260.0, "case_ids": list(CORNERS["D"])},
        },
