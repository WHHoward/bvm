#!/usr/bin/env python3
"""Raw-first analysis for the registered L1/L3/BJ2 closure point."""
...
REFERENCE_CASES = {
    "R0_canonical_L1_1p4_L3_1p3_BJ2_2p0": {
        "0011": EXP / "references/canonical_l3p13/0011/raw.csv",
        "0111": EXP / "references/canonical_l3p13/0111/raw.csv",
    },
    "R1_L1_1p2_L3_1p3_BJ2_2p0": {
        "0011": EXP / "references/l1p12_l3p13_bj2p20/0011/raw.csv",
        "0111": EXP / "references/l1p12_l3p13_bj2p20/0111/raw.csv",
    },
    "R2_L1_1p4_L3_2p0_BJ2_2p2": {
        "0011": EXP / "references/l3p20_bj2p22/0011/raw.csv",
        "0111": EXP / "references/l3p20_bj2p22/0111/raw.csv",
    },
}
...
def post_read_l1_metrics(trace: RawTrace) -> dict[str, Any]:
    values = list(trace.column("I(L1|XBQ1)"))
    selected = indices(trace, 121.0, 200.0)
    threshold = 1.0e-9
    negative_seen = False
    crossing = None
    ...
    for index in selected:
        if values[index] > threshold:
            if start is None:
                start = index
            if negative_seen and crossing is None:
                confirmation = [item for item in selected if item >= index][:3]
                if len(confirmation) == 3 and all(values[item] > threshold for item in confirmation):
                    crossing = trace.time[index] * 1.0e12
        else:
            if start is not None:
                positive_intervals.append((start, index - 1))
                start = None
            if values[index] < -threshold:
                negative_seen = True
    if start is not None:
        positive_intervals.append((start, selected[-1]))
    durations = [trace.time[right] * 1.0e12 - trace.time[left] * 1.0e12 for left, right in positive_intervals]
    return {
        "window_ps": [121.0, 200.0],
        "threshold_A": threshold,
        "peak_post_read_A": max((values[index] for index in selected), default=None),
        "minimum_post_read_A": min((values[index] for index in selected), default=None),
        "first_sustained_negative_to_positive_crossing_ps": crossing,
        "positive_dwell_duration_ps": max(durations, default=0.0),
        "positive_intervals_ps": [[trace.time[left] * 1.0e12, trace.time[right] * 1.0e12] for left, right in positive_intervals],
        "actual_grid_only": True,
    }
...
def case_evidence(trace: RawTrace) -> dict[str, Any]:
    ...
    internal_reset = {
        ...
        "integral_128_132_V_s": trapezoid(trace.time, vdiff, indices(trace, 128.0, 132.0)),
        ...
        "post_read_L1": post_read_l1_metrics(trace),
    }
    return {
...
def fourth_response_record(case: dict[str, Any]) -> dict[str, Any]:
    ...
    item.update({
        ...
        "post_third_reset": {key: case["internal_reset"].get(key) for key in ("peak_I_L1_A", "min_I_L1_A", "first_I_L1_zero_or_sign_change_ps", "integral_118_121_V_s", "integral_121_124_V_s", "integral_124_128_V_s", "integral_128_132_V_s")},
    })
    return item
...
def analyze_pair(point_id: str, changes: dict[str, float], n2_path: Path, n3_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "bvm-qb-l1-l3-bj2-targeted-closure-point-result-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "analysis_version": "v1_strict_all_response_ordering",
        "point_id": point_id,
        "changes": changes,
        ...
        "comparisons": {
            ...
            "comparison_axis": "target L1=1.2/L3=2.0/BJ2=2.2 versus canonical, L1-only rescue and previous L3/BJ2 barrier raw references",
        },
