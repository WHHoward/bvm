#!/usr/bin/env python3
"""Minimal QA for the flat whole-run BJS400 visualization artifacts."""
...
MASK_COMPARISONS = [
    f"{setting}_MASKS_{kind}.html"
    for setting in SETTINGS
    for kind in ("SIGNAL_PATH", "QB", "JTL")
]
COMPARISON_NAMES = MASK_COMPARISONS + [
    f"{setting}_SINGLE_VS_ARRAY1.html" for setting in SETTINGS
] + [
    f"{setting}_BJS300_VS_BJS400_1000.html" for setting in SETTINGS
]
...
def main() -> int:
    ...
    if len(standalone) != 144 or len(comparisons) != 20:
    ...
    qa = {
        "schema": "bjs400-array-single-flat-visualization-qa-v1",
