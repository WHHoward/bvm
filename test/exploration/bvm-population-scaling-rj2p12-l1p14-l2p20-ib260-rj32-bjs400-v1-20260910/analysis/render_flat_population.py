...
REUSE_RUNS = set(RUN_IDS[:2])
RUNS = OrderedDict(
    (run_id, ("references/reused" if run_id in REUSE_RUNS else "runs") + f"/{run_id}")
...
def main() -> int:
    ...
    render_comparison(
        manifest_entries,
        "ALL_SELECTED_MASKS.html",
        cases,
        paths,
        specs["SIGNAL_PATH"],
        EXP / "visualization/plots/comparison/ALL_SELECTED_MASKS.html",
    )
    for name, selected, labels in (
        ("WEIGHT_2_POSITION.html", ["0011", "0110", "1100"], specs["QB_STATE"]),
        ("WEIGHT_3_POSITION.html", ["1110", "0111"], specs["QB_STATE"]),
        ("DOWNSTREAM_TERMINAL_POPULATION.html", cases, specs["JTL_CHAIN"]),
    ):
        selected_cases = [next(run_id for run_id in RUN_IDS if run_id.endswith(f"_{mask}")) for mask in selected] if selected and selected[0].isdigit() else selected
        render_comparison(
            manifest_entries,
            name,
            selected_cases,
            [raw_path(case) for case in selected_cases],
            labels,
            EXP / "visualization/plots/comparison" / name,
        )
    ...
    manifest = {
        "schema": "bjs400-selected-mask-flat-visualization-manifest-v1",
        ...
        "no_extra_masks": True,
        "status": "PASS" if len(standalone) == 42 and len(comparisons) == 4 else "FAIL",
    }
