#!/usr/bin/env python3
"""Execute exactly the eight newly authorized high-range RJ1 runs once."""
...
from prepare_experiment import NEW_RUNS  # noqa: E402
...
def make_record(run_id: str, head: str, command: list[str], started: str, finished: str, exit_code: int, deck: Path, raw: Path, log: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "rj1-highrange-run-metadata-v1",
        "experiment_id": EXP.name,
        "run_id": run_id,
        "topology": "ARRAY",
        "working_point": {"L1_pH": 1.6, "IBias_uA": 260.0, "RJ1_ohm": rj1},
        "rj1_ohm": rj1,
        "mask": mask,
        "parameters": {"L1_pH": 1.6, "IBias_uA": 260.0, "RJ1_ohm": rj1},
...
def main() -> int:
    ...
    if preflight.get("status") != "PASS" or preflight.get("new_physical_solve_count") != 8 or preflight.get("historical_reference_case_count") != 8:
        raise RuntimeError("preflight is not PASS for exact 8-new/8-history matrix")
    ...
    summary = {
        "schema": "rj1-highrange-execution-v1",
        "experiment_id": EXP.name,
    ...
    print(json.dumps({"status": summary["status"], "solver_invocations": len(NEW_RUNS), "new_physical_solve_count": 8, "historical_reference_case_count": 8, "failed_runs": failures, "scientific_analysis_performed": False}, ensure_ascii=False, indent=2))
