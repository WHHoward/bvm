#!/usr/bin/env python3
"""Mechanical QA for boundary-search visualizations."""
...
def main() -> int:
    ...
    if not comparisons:
        failures.append("no focused endpoint/midpoint comparisons")
    ...
    for entry in standalone:
        ...
        if output in outputs:
            failures.append(f"duplicate HTML: {entry.get('output_path')}")
        outputs.add(output)
        ...
        if any(label.startswith("P(") for label in entry.get("labels", [])):
            ...
            if "Phase (turns)" not in html or "rad\\u002f2pi" not in html:
                failures.append(f"phase display conversion missing: {entry.get('output_path')}")
    for run_id, count in per_run.items():
        if count == 0:
            failures.append(f"missing standalone run entry: {run_id}")
    for entry in comparisons:
        ...
        if output in outputs:
            failures.append(f"duplicate comparison HTML: {entry.get('output_path')}")
        outputs.add(output)
    if list(Path("/tmp").glob("bvm-boundary-*.csv")):
    ...
    record: dict[str, Any] = {"schema": "bvm-qb-threshold-ordering-boundary-visualization-qa-v1", "experiment_id": EXP.name, "created_at_local": now(), "status": "PASS" if not failures else "FAIL", "standalone_html_count": len(standalone), "comparison_html_count": len(comparisons), "phase_page_count": phase_pages, "per_run_standalone_counts": per_run, "whole_run_window_ps": [0.0, 200.0], "focused_window_ps": [108.0, 150.0], "temporary_inputs_deleted": not list(Path("/tmp").glob("bvm-boundary-*.csv")), "derived_csv_present": bool(list((EXP / "visualization").rglob("*.csv"))), "raw_direct_standalone": True, "semantic_completeness_gates": {"raw_direct": "PASS" if not failures else "FAIL", "comparison_present": "PASS" if comparisons else "FAIL", "no_unknown_axis": "PASS" if not any("Unknown axis" in item for item in failures) else "FAIL"}, "failures": failures}
