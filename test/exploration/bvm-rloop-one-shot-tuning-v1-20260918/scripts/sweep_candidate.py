...
def write_batch_review(batch_root: Path, sweep: dict[str, Any], rows: list[dict[str, Any]], reused: list[dict[str, Any]], new_cases: list[str], failed: list[dict[str, Any]]) -> None:
    def chart(title: str, keys: list[tuple[str,str]]) -> str:
        return render_svg_chart(rows, "SWEEP_VALUE", keys, title)
    ...
    for r in rows:
        ...
        point_rows.append(f"<tr><td>{r['SWEEP_VALUE']}</td><td>{html.escape(str(r['source_type']))}</td><td>{html.escape(str(r['raw_qa_status']))}</td><td>{r['JS1_read_net']}</td><td>{r['JS1_read_recovery_net']}</td><td>{html.escape(str(r['mechanical_progression_label']))}</td><td>{r['FINAL_JSL8_peak_positive']}</td><td>{r['CONTROL_JSL8_peak_positive']}</td><td>{links}</td></tr>")
    body.append("<section><h2>Point table</h2><table><tr><th>"+html.escape(sweep["key"])+"</th><th>source</th><th>raw QA</th><th>JS1 read</th><th>JS1 read+recovery</th><th>JS1 label</th><th>final peak</th><th>control peak</th><th>links</th></tr>"+"".join(point_rows)+"</table></section><p>No scientific interpretation performed. Phase turns are navigation only, not formal SFQ counts.</p></body></html>")
