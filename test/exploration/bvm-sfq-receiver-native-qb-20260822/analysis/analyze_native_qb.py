#!/usr/bin/env python3
"""Analyze the native paper-QB compatibility Exploration from raw CSVs.
...
def analyze_case(path):
    ...
    for key in ["V(SL1)", "V(N6|XBVM1)", "V(OUT_Q)", "I(L_SL|XBVM1)", "I(L_PSL|XBVM1)"]:
...
def provisional_verdict(cases):
    if not cases or any(not case["artifact"]["valid"] for case in cases):
        return "INVALID"
    read1 = next(case for case in cases if case["case"] == "read1")
    read0 = next(case for case in cases if case["case"] == "read0")
    controls = [case for case in cases if "read0-control" in case["case"]]
    bjl2_read1 = read1["junctions"]["BJL2"]["qualifying_complete_segment_count"]
    bjl2_other = [case["junctions"]["BJL2"]["qualifying_complete_segment_count"] for case in [read0, *controls]]
    if bjl2_read1 == 1 and all(count == 0 for count in bjl2_other):
        return "NATIVE_QB_LOCAL_PASS_CANDIDATE"
    read1_ranges = [read1["junctions"][name]["activity_range_turn"] for name in JUNCTIONS]
    other_ranges = [
        case["junctions"][name]["activity_range_turn"]
        for case in [read0, *controls]
        for name in JUNCTIONS
    ]
    if max(read1_ranges, default=0.0) > 0.02 and max(read1_ranges, default=0.0) > 2.0 * max(other_ranges, default=1.0e-12):
        return "STATE_SELECTIVE_QB_ACTIVITY_CANDIDATE"
    if max(read1_ranges, default=0.0) < 0.02 and read1["branches"]["Lin"]["peak_abs_uA"] < 133.0:
        return "SOURCE_TOO_WEAK_OR_SCALE_MISMATCH_CANDIDATE"
    return "UNRESOLVED_MANUAL_CLASSIFICATION"
...
def main():
    ...
    output = OUT / "native-qb-summary.json"
