#!/usr/bin/env python3
"""Register the fixed L1/IBias RJ1 interaction experiment.

This script performs no physical solve.  It creates the six registered RJ1
decks and verifies the exact RJ1=12 ohm historical baseline before execution.
"""
...
AUTH = REPO / "test/exploration/qb-l1-ibias-full-grid-bjs400-v1-20260910"
...
AUTH_SOURCE_MANIFEST_SHA256 = "8166b59ae696a9d22d844f0736e452f354aa32e0a92fbb4d62396998d0c79fa7"
AUTH_PACKAGE_SHA256 = "7ccb82a479f792a14e8f1e6bbe66f7f8acd51ef133f6fbe23175b74306c0ab18"
...
def write_reuse_manifest() -> dict[str, Any]:
    ...
    if not AUTH.is_dir() or sha256(AUTH / "SOURCE_MANIFEST.json") != AUTH_SOURCE_MANIFEST_SHA256:
        raise RuntimeError("full-grid source manifest is missing or changed")
    authority_package = AUTH / "handoff/qb-l1-ibias-full-grid-bjs400-v1-20260910_raw_handoff_v2.zip"
    if not authority_package.is_file() or sha256(authority_package) != AUTH_PACKAGE_SHA256:
        raise RuntimeError("full-grid source package is missing or changed")
    ...
    for run_id, spec in SOURCE_REUSE.items():
        ...
        expected_deck = build_deck(12.0, spec["mask"])
        ...
        if normalized_deck(source_deck) != normalized_deck(expected_deck):
            failures.append("deck/include/topology mismatch")
        ...
        if failures:
            raise RuntimeError(f"RJ1=12 exact reuse verification failed for {run_id}: {'; '.join(failures)}")
        references[run_id] = {
    ...
    record = {
        "schema": "bjs400-rj1-interaction-reuse-manifest-v1",
...
def write_source_manifest() -> dict[str, Any]:
    ...
    record = {
        "schema": "bjs400-rj1-interaction-source-manifest-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
...
def generate_decks() -> dict[str, Any]:
    ...
    for run_id in NEW_RUNS:
        ...
        records[run_id] = {"rj1_ohm": RUN_TO_RJ1[run_id], "mask": RUN_TO_MASK[run_id], "parameters": {**FIXED, "RJ1_ohm": RUN_TO_RJ1[run_id]}, "path": str(deck.relative_to(REPO)), "sha256": sha256(deck), "bytes": deck.stat().st_size}
    return records
...
def verify_decks() -> dict[str, Any]:
    ...
    return {
        "schema": "bjs400-rj1-interaction-deck-diff-preflight-v1",
...
def preflight_markdown(record: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "",
        f"- RJ1=12 reuse comparability: `{record['reuse_comparability']['status']}`.",
...
def main() -> int:
    ...
    record = {
        "schema": "bjs400-rj1-interaction-preflight-v1",
        "experiment_id": EXP.name,
        "created_at_local": now(),
        "status": "PASS",
        "head": head(),
