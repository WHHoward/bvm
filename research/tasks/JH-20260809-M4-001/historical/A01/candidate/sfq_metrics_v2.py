...
physical Gate (no PASS/FAIL for BQ/DCSFQ/JTL/system claims). Metric spec
tolerances are NOT frozen until M9 (METRIC_SPEC_V2).

Naming (AC3): per-sample threshold statistics are reported only as
``over_threshold_sample_count`` and ``activity_intervals`` (sample /
interval semantics). No ``fast_events``, ``pulse_count`` or ``sfq_count``
style event semantics are produced.
"""
...
def _over_threshold_stats(
    ...
    diffs = [
        abs(phase[i + 1] - phase[i]) for i in range(len(phase) - 1)
    ]
...
def analyze(csv_path: str, threshold_rad: float = DEFAULT_THRESHOLD_RAD) -> dict[str, Any]:
    """Extract M4 unit-level metrics from a JoSIM phase-mode CSV.

    Returns a JSON-ready dict that preserves raw rad and derives turns.
    No event semantics are inferred from samples.
    """
    ...
    for c in phase_cols:
        ...
        junctions[c] = {
            ...
            **_over_threshold_stats(p, times, threshold_rad),
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--json",
        metavar="OUT",
        help="write JSON to OUT instead of stdout",
    )
